...
switch ( $eparam['mode'] ) {
	...
	// delete from database at once
		////$photo->uid=$diary->uid;
		//$photo->bid  = $diary->bid;
		//$photo->pids = $pdels;
		//$photo->deletedbF_mul($mydirname);
	...
	foreach ( $pdels as $pdel ) {
		//$del_pid = addslashes($d3dConf->func->getpost_param('del_pid'));
		$del_pid = addslashes($pdel);
		$pattern = array("..","index.html",".php");
		$replace = array("","","");
		$trim_pid = str_replace($eparam['previewdir'], "", $del_pid);

		//$del_pname = $d3dConf->func->getpost_param('del_pname');
