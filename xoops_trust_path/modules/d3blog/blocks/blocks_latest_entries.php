 ...
 * $options[8] = cids
 * $options[9] = uids
 * $options[10] = original template file
 */
...
function b_d3blog_latest_entries_show($options) {
    ...
    $opt_cid = preg_replace('/[^0-9,]/', '', @$options[8]);
    $opt_uid = preg_replace('/[^0-9,]/', '', @$options[9]);
    $opt_template = empty( $options[10] ) ? 'db:'.$mydirname.'_block_latest_entries.html' : trim( $options[10] );
    ...
    if($currentUser->blog_perm_view($myModule->module_id)) {
        ...
        // blogger filter
        if(isset($_GET['uid'])) {   // in case myblog
            $opt_uid .= !empty($opt_uid)? ',' : '';
            $opt_uid .= intval($_GET['uid']);
        }
        if(!empty($opt_uid)) {
            $criteria->add(new criteria('uid', '('. $opt_uid. ')', 'IN'));
        }
        // category filter
        if(!empty($opt_cid)) {
            $criteria->add(new criteria('cid', '('. $opt_cid. ')', 'IN'));
        }
    ...
    if(empty($options['disable_renderer'])) {
        ...
        $ret['content'] = $tpl->fetch($opt_template);
        return $ret ;
    } else {
        return $block ;
    }
...
function b_d3blog_latest_entries_edit($options) {
    ...
    $form .= sprintf('<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%s" /></td></tr>',
                     _MB_D3BLOG_EDIT_CATEGORY_FILTER, htmlspecialchars($options[8], ENT_QUOTES));
    $form .= sprintf('<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%s" /></td></tr>',
                     _MB_D3BLOG_EDIT_USER_FILTER, htmlspecialchars($options[9], ENT_QUOTES));
    $form .= sprintf('<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%s" /></td></tr>',
                     _MB_D3BLOG_EDIT_TEMPLATE, htmlspecialchars($options[10], ENT_QUOTES));
    $form .= '</table>';
    return $form;
}
