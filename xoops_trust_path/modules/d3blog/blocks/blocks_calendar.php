<?php
/**
 * @version $Id: blocks_calendar.php 541 2008-10-07 05:25:42Z hodaka $
...
function b_d3blog_calendar_show($options) {
    ...
    if(!empty($uid)) {
        $criteria->add(new criteria('uid', intval($uid)));
    }
    
    $month_unixtime = mktime(0, 0, 0, $this_month, 1, $this_year);
    $criteria->add(new criteria('published', $month_unixtime - $currentUser->timeoffset(), '>='));
    $month_unixtime = mktime(0, 0, 0, $this_month + 1, 1, $this_year);
    $criteria->add(new criteria('published', $month_unixtime - $currentUser->timeoffset(), '<'));
    ...
    foreach($objs as $obj) {
        ...
        $entries[intval($day)] =& $obj->getArray();
