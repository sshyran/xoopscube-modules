...
function cubookmarken_search($keyword, $andor, $limit, $offset, $userid)
{
	global $xoopsDB;
	$ret = array();
	...
	$module =& $module_handler->getByDirname('cubookmarken');
	...
	$sqlB = "SELECT bm_id, bm_title, uid, memo, reg_unixtime FROM ".$xoopsDB->prefix("cubookmarken_bm")." WHERE ". ($whr_uid);
	...
	/*---------------------------------------------------------------
	 Tag
	 --------------------------------------------------------------*/
	$sqlT = "SELECT bm_id, tag_name, uid, reg_unixtime FROM ".$xoopsDB->prefix("cubookmarken_tag")." WHERE ". ($whr_uid);
	// because count() returns 1 even if a supplied variable
	// is not an array, we must check if $querryarray is really an array
	$count = count($keyword);
	if ( $count > 0 && is_array($keyword) ) {
