...
while (list($d) = $xoopsDB->fetchRow($res)) {
	if ((0 < substr_count($d, ' ')) && (substr_count($d, ' ') < 4)){
		...
		foreach ($q_array as $q) {
			...
			if (strlen($q) > 20) {
				if (!isset($qlong[$q])) { $qlong[$q] = 1; } else { $qlong[$q]++; }
			} elseif ($q != '') {
				if (!isset($qword[$q])) { $qword[$q] = 1; } else { $qword[$q]++; }
			}
		}
	} else {
		if (strlen($d) > 20) {
			if (!isset($qlong[$d])) { $qlong[$d] = 1; } else { $qlong[$d]++; }
		} elseif (trim($d) != '') {
			if (extension_loaded('mbstring')) {
			...
			if (!isset($qword[$d])) { $qword[$d] = 1; } else { $qword[$d]++; }
		}
	}
}
unset($res);
...
print '<tr><td class="head">Count</td><td class="head">Word';
$PNum = 0;
$C = ' class="even"';
foreach ($qword as $key => $val) {
	if ($PNum != $val) {
		...
		print '</td></tr>'.
		'<tr'.$C.'><td align="right">'.$val.'</td><td>';
		$PNum = $val;
		$cnt = 0;
		$C = (($C == ' class="even"') ? ' class="odd"' : ' class="even"');
	} else {
		print ', ';
		if ($cnt++ > 6) { print '<br />'; $cnt = 0; }
	}
	print htmlspecialchars($key, ENT_QUOTES);
}
print '</td></tr>';

print '<tr><td class="head">Count</td><td class="head">Long Text';
$PNum = 0;
$C = ' class="even"';
