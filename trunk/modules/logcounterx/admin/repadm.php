...
$nam = array('REP_R2' => _LCX_ADM_BY_R2, 'REP_QW' => _LCX_ADM_BY_QW, 'REP_OS' => _LCX_ADM_BY_OS,
...
if (isset($_POST['command']) && ($_POST['command'] == 'edit')) {
	$sql = "UPDATE ".$xoopsDB->prefix("logcounterx_cfg")." SET cfgvalue = '".intval($_POST['REP_ROWLIMIT'])."'".
	       " WHERE cfgname = 'REP_ROWLIMIT'";
	$res = $xoopsDB->queryF($sql);
	$sql = "UPDATE ".$xoopsDB->prefix("logcounterx_cfg")." SET cfgvalue = '".intval($_POST['REP_ROWLIMIT_ADM'])."'".
	       " WHERE cfgname = 'REP_ROWLIMIT_ADM'";
	$res = $xoopsDB->queryF($sql);
	$sql = "UPDATE ".$xoopsDB->prefix("logcounterx_cfg")." SET cfgvalue = '".intval($_POST['REP_ROWLIMIT_USR'])."'".
	       " WHERE cfgname = 'REP_ROWLIMIT_USER'";
	$res = $xoopsDB->queryF($sql);
	foreach ($fld as $f) {
		if ($_POST[$f] != $CONF[$f]) {
			$sql = "UPDATE ".$xoopsDB->prefix("logcounterx_cfg")." SET cfgvalue = '".intval($_POST[$f]).
			       "' WHERE cfgname = '$f'";
			$res = $xoopsDB->queryF($sql);
		}
	}
