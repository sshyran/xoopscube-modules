...
class LegacyRender_DD4themePreload extends XCube_ActionFilter
{
	...
	function postFilter()
	{
		...
		$current_bp_cookie = $this->bp_get_module_cookie ($groups_prefix.'dd4theme', $dirname); 

		$lblock4cookie = "nocookie";
		$ccblock4cookie = "nocookie";
		$clblock4cookie = "nocookie";
		$crblock4cookie = "nocookie";
		$rblock4cookie = "nocookie";

		if ($current_bp_cookie != 'nocookie') {
		$count = count($current_bp_cookie);
		for ($i=1; $i<$count; $i++ ) {
		$tmp = explode(':', $current_bp_cookie[$i]);
		if ( $tmp[0] == 'ldropzone') {$lblock4cookie = $tmp[1];}				
		if ( $tmp[0] == 'ccdropzone') {$ccblock4cookie = $tmp[1];}				
		if ( $tmp[0] == 'cldropzone') {$clblock4cookie = $tmp[1];}				
		if ( $tmp[0] == 'crdropzone') {$crblock4cookie = $tmp[1];}				
		if ( $tmp[0] == 'rdropzone') {$rblock4cookie = $tmp[1];}			
		}
		}

		$renderSystem->mXoopsTpl->assign('groupsprefix', $groups_prefix);

		$renderSystem->mXoopsTpl->assign('lblocks4cookie', explode('&', $lblock4cookie));
		$renderSystem->mXoopsTpl->assign('ccblocks4cookie', explode('&', $ccblock4cookie));
		$renderSystem->mXoopsTpl->assign('clblocks4cookie', explode('&', $clblock4cookie));
		$renderSystem->mXoopsTpl->assign('crblocks4cookie', explode('&', $crblock4cookie));
		$renderSystem->mXoopsTpl->assign('rblocks4cookie', explode('&', $rblock4cookie));
