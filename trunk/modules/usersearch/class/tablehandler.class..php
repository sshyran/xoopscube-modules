...
if ( !class_exists('WhereComp') ) {
  require XOOPS_MODULE_PATH.'/usersearch/class/where.class.php';
}
class UsersearchTableObjectHandler
{
  protected $mTable;
  protected $mPrimary;
  protected $mClass;
  ...
  public function get($id)
  {
    ...
    if ( is_array($id) ) {
      foreach ( $this->mPrimary as $key => $pkey ) {
        $WhereComp->add(new WhereElement(1, $pkey, $id[$key]));
      }
    } else {
  ...
  private function _makeVars4sql(&$obj)
  {
    $ret = array();
