...
require_once XOOPS_MODULE_PATH . "/xcat/class/AbstractAction.class.php";

define('XCAT_FRAME_PERFORM_SUCCESS', 1);
define('XCAT_FRAME_PERFORM_FAIL', 2);
define('XCAT_FRAME_INIT_SUCCESS', 3);

define('XCAT_FRAME_VIEW_NONE', "none");
define('XCAT_FRAME_VIEW_SUCCESS', "success");
define('XCAT_FRAME_VIEW_ERROR', "error");
define('XCAT_FRAME_VIEW_INDEX', "index");
define('XCAT_FRAME_VIEW_INPUT', "input");
define('XCAT_FRAME_VIEW_PREVIEW', "preview");
define('XCAT_FRAME_VIEW_CANCEL', "cancel");

class Xcat_Module extends Legacy_ModuleAdapter
{
	...
	function startup()
	{
		...
		XCube_DelegateUtils::call('Module.xcat.Event.GetAssetManager', new XCube_Ref($this->mAssetManager));
	...
	function execute(&$controller)
	{
		...
		if ($this->mAdminFlag) {
			$className = "Xcat_Admin_" . ucfirst($this->mActionName) . "Action";
			$fileName = XOOPS_MODULE_PATH . "/xcat/admin/actions/${fileName}.class.php";
		}
		else {
			$className = "Xcat_" . ucfirst($this->mActionName) . "Action";
			$fileName = XOOPS_MODULE_PATH . "/xcat/actions/${fileName}.class.php";
		}
		...
		if ($this->mAction->isAdminOnly() && !$controller->mRoot->mContext->mUser->isInRole('Module.xcat.Admin')) {
			$this->doPermissionError();
			die();
		}
	...
	{
		XCube_DelegateUtils::call("Module.Xcat.Event.Exception.Permission");
		$root =& XCube_Root::getSingleton();
	...
	{
		XCube_DelegateUtils::call("Module.Xcat.Event.Exception.ActionNotFound");
		$root =& XCube_Root::getSingleton();
		$root->mController->executeForward(XOOPS_URL);
		return;
	}
	...
	{
		XCube_DelegateUtils::call("Module.Xcat.Event.Exception.Preparation");
		$root =& XCube_Root::getSingleton();
		$root->mController->executeForward(XOOPS_URL);
		return;
	}
