<?php
	...
	*/
	...
	$result = $db->query( "SELECT title,lid,`date`,cid FROM ".$db->prefix( $plugin['dirname']."_downloads" )." WHERE ( $whr_cat4read ) AND ( $whr_cat ) AND visible = '1' AND `date` >= $range_start_s AND `date` < $range_end_s" ) ;
