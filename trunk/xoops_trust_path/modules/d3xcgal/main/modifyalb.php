...
include dirname(dirname(__FILE__)).'/include/init.inc.php' ;
...
function get_subcat_data($parent, $ident='')
{
    ...
    $mydirname = $xoopsModule->getVar('dirname') ;
        ...
        $result = $xoopsDB->query("SELECT cid, name, description FROM ".$xoopsDB->prefix($mydirname."_categories")." WHERE parent = '$parent' AND cid != 1 ORDER BY pos");
...
{
        global $album, $xoopsModule, $xoopsDB;
        $mydirname = $xoopsModule->getVar('dirname') ;
        ...
        if (GALLERY_ADMIN_MODE) {
                ...
                $result = $xoopsDB->query("SELECT aid, title FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category = '".(FIRST_USER_CAT + USER_ID)."' ORDER BY title");
        }
    $user_handler =& xoops_gethandler('member');
        if ($xoopsDB->getRowsNum($result) > 0 ){
                $lb = "<select name=\"album_listbox\" class=\"listbox\" onChange=\"if(this.options[this.selectedIndex].value) window.location.href='index.php?page=modifyalb&album='+this.options[this.selectedIndex].value;\" />\n";
...
if (!isset($_GET['album'])) {
        if (GALLERY_ADMIN_MODE) {
            $results = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE 1 LIMIT 1");
        } else {
            $results = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category = ".(FIRST_USER_CAT + USER_ID)." LIMIT 1");
        }
        if ($xoopsDB->getRowsNum($results) == 0) redirect_header('index.php',2,_MD_D3XCGAL_MODIFYALB_ERR_NO_ALB);
        $ALBUM_DATA = $xoopsDB->fetchArray($results);
        $album = $ALBUM_DATA['aid'];
} else {
        $album = (int)$_GET['album'];
        $results = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE aid='$album'");
        if(!$xoopsDB->getRowsNum($results)) redirect_header('index.php',2,_MD_D3XCGAL_NON_EXIST_AP);
        $ALBUM_DATA = $xoopsDB->fetchArray($results);
}
...
if (!GALLERY_ADMIN_MODE && $ALBUM_DATA['category'] != FIRST_USER_CAT + USER_ID) {
    redirect_header('index.php',2,_MD_D3XCGAL_PERM_DENIED);
}
$xoopsOption['template_main'] = $mydirname.'_modifyalb.html';
include XOOPS_ROOT_PATH."/header.php";
$xoopsTpl->assign('mydirname',$mydirname);
$xoopsTpl->assign('xoops_module_header', $d3xcgal_module_header);
$album_lb =alb_list_box();
$xoopsTpl->assign('update', _MD_D3XCGAL_MODIFYALB_UPDATE);
$xoopsTpl->assign('album_lb', $album_lb);
$xoopsTpl->assign('album', $album);

$xoopsTpl->assign('general_settings', _MD_D3XCGAL_MODIFYALB_GEN_SET);
$xoopsTpl->assign('alb_title', _MD_D3XCGAL_MODIFYALB_ALB_TITLE);
$xoopsTpl->assign('album_data_title', $ALBUM_DATA['title']);
$xoopsTpl->assign('alb_cat', _MD_D3XCGAL_MODIFYALB_ALB_CAT);
if (!GALLERY_ADMIN_MODE || $ALBUM_DATA['category'] > FIRST_USER_CAT){
        $xoopsTpl->assign('user_galleries', 1);
        $xoopsTpl->assign('user_gal', _MD_D3XCGAL_MODIFYALB_USER_GAL);
        $xoopsTpl->assign('album_data_category', $ALBUM_DATA['category']);
        }
else {
        $CAT_LIST = array();
        $CAT_LIST[] = array(0, _MD_D3XCGAL_MODIFYALB_NO_CAT);
        get_subcat_data(0,'');
    $cat_list='';
...
$xoopsTpl->assign('alb_desc', _MD_D3XCGAL_MODIFYALB_ALB_DESC);
$xoopsTpl->assign('album_data_description', $ALBUM_DATA['description']);
$xoopsTpl->assign('alb_thumb', _MD_D3XCGAL_MODIFYALB_ALB_THUMB);
$results=$xoopsDB->query("SELECT pid, filepath, filename, url_prefix FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE aid='$album' AND approved='YES' ORDER BY filename");
if ($xoopsDB->getRowsNum($results) == 0) {
    $xoopsTpl->assign('no_pic', 1);
    $xoopsTpl->assign('alb_empty', _MD_D3XCGAL_MODIFYALB_ALB_EMPTY);
    }
else {
    $initial_thumb_url = 'images/nopic.jpg';
        $img_list=array(0 => _MD_D3XCGAL_MODIFYALB_LAST_UPL);
        $pic_url='';
        ...
        while($picture=$xoopsDB->fetchArray($results)){
                ...
                $img_list[$picture['pid']] = htmlspecialchars($picture['filename'], ENT_QUOTES);
        }
...
$xoopsTpl->assign('alb_perm',_MD_D3XCGAL_MODIFYALB_ALB_PERM);
$xoopsTpl->assign('can_view',_MD_D3XCGAL_MODIFYALB_CAN_VIEW);
if (!$xoopsModuleConfig['allow_private_albums']) {
    ...
    if (GALLERY_ADMIN_MODE) {
                   $options = array(0 => _MD_D3XCGAL_MODIFYALB_PUB_ALB,FIRST_USER_CAT + USER_ID => _MD_D3XCGAL_MODIFYALB_ME_ONLY);
                    ...
                    $result = $xoopsDB->query("SELECT uname FROM ".$xoopsDB->prefix("users")." WHERE uid='".($ALBUM_DATA['category'] - FIRST_USER_CAT)."'");
                        if ($xoopsDB->getRowsNum($result)) {
                            $user = $xoopsDB->fetchArray($result);
                                $options[$ALBUM_DATA['category']] = sprintf(_MD_D3XCGAL_MODIFYALB_OWNER_ONLY, $user['uname']);
                        }
                }
                $result = $xoopsDB->query("SELECT group_id, group_name FROM ".$xoopsDB->prefix($mydirname."_usergroups")." WHERE 1");
                while($group = $xoopsDB->fetchArray($result)){
                        $options[$group['group_id']] = sprintf(_MD_D3XCGAL_MODIFYALB_GROUP_ONLY, $group['group_name']);
                } // while
        } else {
                $options = array(
                        0 => _MD_D3XCGAL_MODIFYALB_PUB_ALB,
                        FIRST_USER_CAT + USER_ID => _MD_D3XCGAL_MODIFYALB_ME_ONLY,
                );
            ...
            $options[$ugr] = sprintf(_MD_D3XCGAL_MODIFYALB_GROUP_ONLY, $name);
...
$xoopsTpl->assign('can_upload_pic',_MD_D3XCGAL_MODIFYALB_CAN_UPLOAD);
if (USER_ADMIN_MODE){
...
$xoopsTpl->assign('can_post_comments', _MD_D3XCGAL_MODIFYALB_CAN_COM);
...
$xoopsTpl->assign('can_rate', _MD_D3XCGAL_MODIFYALB_CAN_RATE);
