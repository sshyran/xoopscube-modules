...
function form_alb_list_box()
{
    ...
    //var_dump($other_user_albums_list);
        ...
        $box.= "<optgroup label=\""._MD_D3XCGAL_UPL_YOURALB."\">";
        ...
        $box.= "<optgroup label=\""._MD_D3XCGAL_UPL_ALBPUB."\">";
    ...
    $box.= "<optgroup label=\""._MD_D3XCGAL_UPL_OUSERALB."\">";
...
if (GALLERY_ADMIN_MODE) {
    $public_albums = $xoopsDB->query("SELECT aid, title FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category < ".FIRST_USER_CAT." ORDER BY title");
        ...
        $public_albums = $xoopsDB->query("SELECT aid, title FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category < ".FIRST_USER_CAT." AND uploads='YES' ORDER BY title");
}
...
if (USER_ID) {
    ...
    $other_user=$xoopsDB->query("SELECT aid, title, category FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category > ".FIRST_USER_CAT." AND uploads='YES' AND category!='".(FIRST_USER_CAT + USER_ID)."' $ALBUM_SET ORDER BY category");
...
if (!count($public_albums_list) && !count($user_albums_list) && !USER_CAN_CREATE_ALBUMS) {
    redirect_header('index.php',2,_MD_D3XCGAL_UPL_ERR_NO_ALB_UPLOAD);
} elseif (!count($public_albums_list) && !count($user_albums_list) && USER_CAN_CREATE_ALBUMS) {
        $USER['am'] = 1;
        $redirect = "index.php?page=albmgr";
        redirect_header($redirect,2,_MD_D3XCGAL_UPL_ERR_NO_ALB_UPLOAD);
        exit;
}

$xoopsOption['template_main'] = $mydirname.'_uploadmore.html';
...
ob_end_clean();
$xoopsTpl->assign('max_upl',sprintf(_MD_D3XCGAL_UPL_MAX_FSIZE, $xoopsModuleConfig['max_upl_size']));
$xoopsTpl->assign('lang_upload', _MD_D3XCGAL_UPL_TITLE);
$xoopsTpl->assign('lang_album', _MD_D3XCGAL_ALBUM);
$xoopsTpl->assign('lang_picture', _MD_D3XCGAL_UPL_PICTURE);
$xoopsTpl->assign('lang_picture_title', _MD_D3XCGAL_UPL_PIC_TITLE);
$xoopsTpl->assign('lang_keywords', _MD_D3XCGAL_UPL_KEYWORDS);
$xoopsTpl->assign('max_file_size',$xoopsModuleConfig['max_upl_size']<<10);
$xoopsTpl->assign('lang_options', "_MD_D3XCGAL_OPTIONS");
$xoopsTpl->assign('lang_notify', "_MD_D3XCGAL_NOTIFYAPPROVE");
$xoopsTpl->assign('lang_description', _MD_D3XCGAL_UPL_DESCRIPTION);
