...
if (!(USER_IS_ADMIN || USER_ADMIN_MODE)) redirect_header('index.php',2,_MD_D3XCGAL_ACCESS_DENIED);
...
if (UPLOAD_APPROVAL_MODE && !USER_IS_ADMIN) redirect_header('index.php',2,_MD_D3XCGAL_ACCESS_DENIED);

if (EDIT_PICTURES_MODE) {
    $result = $xoopsDB->query("SELECT title, category FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE aid = '$album_id'");
        ...
        $actual_cat = $cat;
        if ($cat != FIRST_USER_CAT + USER_ID && !GALLERY_ADMIN_MODE) redirect_header('index.php',2,_MD_D3XCGAL_PERM_DENIED);
} else {
        $ALBUM_DATA = array();
}
...
function get_post_var($var, $pid)
{
        ...
        $var_name = $var.$pid;
        if(!isset($_POST[$var_name])) redirect_header('index.php',2,_MD_D3XCGAL_PARAM_MISSING." ($var_name)");
...
function process_post_data()
{
        ...
        if (!is_array($_POST['pid'])) redirect_header('index.php',2,_MD_D3XCGAL_PARAM_MISSING);
        $pid_array = &$_POST['pid'];
        foreach($pid_array as $pid){
                ...
                $query = "SELECT category, filepath, filename, owner_id FROM ".$xoopsDB->prefix($mydirname."_pictures").", ".$xoopsDB->prefix($mydirname."_albums")." WHERE ".$xoopsDB->prefix($mydirname."_pictures").".aid = ".$xoopsDB->prefix($mydirname."_albums").".aid AND pid='$pid'";
                ...
                if (!USER_IS_ADMIN) {
                        if ($pic['category'] != FIRST_USER_CAT + USER_ID) redirect_header('index.php',2, _MD_D3XCGAL_PERM_DENIED."<br />(picture category = {$pic['category']}/ $pid)");
                        if (!isset($user_album_set[$aid])) redirect_header('index.php',2,_MD_D3XCGAL_PERM_DENIED."<br />(target album = $aid)");
                }
                ...
                if ($del_comments) {
                        //$query = "DELETE FROM ".$xoopsDB->prefix($mydirname."_comments")." WHERE pid='$pid'";
                        //$result =$xoopsDB->query($query);
                ...
                if ($delete) {
                        ...
                        if (!is_writable($dir)) redirect_header('index.php',2, sprintf(_MD_D3XCGAL_DIRECTORY_RO, $dir));
                        ...
                        $query = "DELETE FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE pid='$pid' LIMIT 1";
                        $result = $xoopsDB->query($query);
                } else {
                        $query = "UPDATE ".$xoopsDB->prefix($mydirname."_pictures")." SET $update WHERE pid='$pid' LIMIT 1";
                        $result = $xoopsDB->query($query);
                        if ($pic['owner_id'] != 0){
...
function form_pic_info()
{
    ...
    global $filename, $pic_info, $thumb_url, $thumb_link,$myts;
        ...
        if (UPLOAD_APPROVAL_MODE) {
            $pic_info = $CURRENT_PIC['pwidth'].'x'.$CURRENT_PIC['pheight'].' - '.($CURRENT_PIC['filesize'] >> 10)._MD_D3XCGAL_KB;
        ...
        if (is_object ($pic_owner)){
                ...
                $pic_info = sprintf(_MD_D3XCGAL_EDITPICS_INFOSTR, $CURRENT_PIC['pwidth'], $CURRENT_PIC['pheight'], ($CURRENT_PIC['filesize'] >> 10), $CURRENT_PIC['hits'], $CURRENT_PIC['votes']);
        }

        $thumb_url = get_pic_url($CURRENT_PIC, 'thumb');
        $thumb_link = 'index.php?page=displayimage&pid='.$CURRENT_PIC['pid'].'&amp;pos='.(-$CURRENT_PIC['pid']);
...
function form_options()
{
        ...
        if (UPLOAD_APPROVAL_MODE) {
                $pic_opt= "
                        <b><input type=\"radio\" name=\"approved{$CURRENT_PIC['pid']}\" value=\"YES\" class=\"radio\" />"._MD_D3XCGAL_EDITPICS_APPROVE."</b>&nbsp;
                        <b><input type=\"radio\" name=\"approved{$CURRENT_PIC['pid']}\" value=\"NO\" class=\"radio\" checked=\"checked\" />"._MD_D3XCGAL_EDITPICS_PP_APPROVE."</b>&nbsp;
                        <b><input type=\"radio\" name=\"approved{$CURRENT_PIC['pid']}\" value=\"DELETE\" class=\"radio\" />"._MD_D3XCGAL_EDITPICS_DEL_PIC."</b>&nbsp;";
        } else {
                $pic_opt= "
                        <b><input type=\"checkbox\" name=\"delete{$CURRENT_PIC['pid']}\" value=\"1\" class=\"checkbox\" />"._MD_D3XCGAL_EDITPICS_DEL_PIC."</b>&nbsp;
                        <b><input type=\"checkbox\" name=\"reset_vcount{$CURRENT_PIC['pid']}\" value=\"1\" class=\"checkbox\" />"._MD_D3XCGAL_EDITPICS_RVIEW."</b>&nbsp;
                        <b><input type=\"checkbox\" name=\"reset_votes{$CURRENT_PIC['pid']}\" value=\"1\" class=\"checkbox\" />"._MD_D3XCGAL_EDITPICS_RVOTES."</b>&nbsp;
                        <b><input type=\"checkbox\" name=\"del_comments{$CURRENT_PIC['pid']}\" value=\"1\" class=\"checkbox\" />"._MD_D3XCGAL_EDITPICS_DCOM."</b>&nbsp;";
        }
}
...
{
        global $USER_ALBUMS_ARRAY, $user_albums_list, $xoopsModule, $xoopsDB;
        $mydirname = $xoopsModule->getVar('dirname') ;

        if (!isset($USER_ALBUMS_ARRAY[$user_id])) {
                $user_albums = $xoopsDB->query("SELECT aid, title FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category='".(FIRST_USER_CAT + $user_id)."' ORDER BY title");
...
if (USER_IS_ADMIN) {
    $public_albums = $xoopsDB->query("SELECT aid, title FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category < '".FIRST_USER_CAT."' ORDER BY title");
...
$next_target = 'index.php?page=editpics&amp;album='.$album_id.'&amp;start='.($start+$count).'&amp;count='.$count;
$prev_target = 'index.php?page=editpics&amp;album='.$album_id.'&amp;start='.max(0,$start-$count).'&amp;count='.$count;
...
if (UPLOAD_APPROVAL_MODE) {
    ...
    $sql =  "SELECT * ".
        ...
        $form_target = 'index.php?page=editpics&amp;mode=upload_approval&amp;start='.$start.'&amp;count='.$count;
        $title = _MD_D3XCGAL_EDITPICS_UPL_APPROVAL;
} else {
        $result=$xoopsDB->query("SELECT count(*) FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE aid = '$album_id'");
        $nbEnr = $xoopsDB->fetchArray($result);
        $pic_count = $nbEnr['count(*)'];
        $xoopsDB->freeRecordSet($result);

    $result = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE aid = '$album_id' ORDER BY filename LIMIT $start, $count");
        $form_target = 'index.php?page=editpics&amp;album='.$album_id.'&amp;start='.$start.'&amp;count='.$count;
        $title = _MD_D3XCGAL_EDITPICS_EDIT;
}
global $HTTP_REFERER;
if (!$xoopsDB->getRowsNum($result)) redirect_header('admin/index.php',2,_MD_D3XCGAL_NO_IMG_TO_DISPLAY);

if ($start + $count < $pic_count) {
    $next_link = "<a href=\"$next_target\"><b>"._MD_D3XCGAL_EDITPICS_NEXT."</b></a>&nbsp;&nbsp;-&nbsp;&nbsp;";
} else {
        $next_link = '';
}

if ($start > 0) {
    $prev_link = "<a href=\"$prev_target\"><b>"._MD_D3XCGAL_EDITPICS_PREV."</b></a>&nbsp;&nbsp;-&nbsp;&nbsp;";
} else {
        $prev_link = '';
}

$pic_count_text = sprintf(_MD_D3XCGAL_NPICS, $pic_count);
$xoopsOption['template_main'] = $mydirname.'_editpics.html';
...
$xoopsTpl->assign('n_of_pic_to_disp',_MD_D3XCGAL_EDITPICS_NUMDIS);
...
$form='';
$xoopsTpl->assign('lang_pic_info',_MD_D3XCGAL_EDITPICS_PIC_INFO);
$xoopsTpl->assign('rowspan',$THUMB_ROWSPAN);
$xoopsTpl->assign('lang_album',_MD_D3XCGAL_ALBUM);
$xoopsTpl->assign('lang_title',_MD_D3XCGAL_EDITPICS_TITLE);
$xoopsTpl->assign('lang_desc',_MD_D3XCGAL_EDITPICS_DESC);
$xoopsTpl->assign('lang_keywords',_MD_D3XCGAL_KEYS);
...
$xoopsTpl->assign('apply',_MD_D3XCGAL_EDITPICS_APPLY);
user_save_profile();
