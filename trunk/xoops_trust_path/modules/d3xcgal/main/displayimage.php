...
if($xoopsModuleConfig['read_exif_data'] && function_exists('exif_read_data')){
        include(dirname(dirname(__FILE__)).'/include/exif_php.inc.php');
} elseif($xoopsModuleConfig['read_exif_data']){
    redirect_header('index.php',2,_MD_D3XCGAL_DIS_EXIF_ERR);
    exit();
}
...
function html_img_nav_menu()
{
    ...
    global $xoopsTpl, $album_name;
        ...
        $human_pos = $pos + 1;
        $spage = ceil(($pos+1) / ($xoopsModuleConfig['thumbrows'] * $xoopsModuleConfig['thumbcols']));
        ...
        if ($pos > 0) {
                ...
                $prev_tgt = "index.php?page=displayimage&amp;album=$album$cat_link&amp;pos=$prev&amp;pid=".$prev_data[0]['pid'];
                $prev_title = _MD_D3XCGAL_DIS_PREV;
        } else {
                $prev_tgt = "javascript:;";
                $prev_title = "";
        }
        if ($pos < ($pic_count -1)) {
                ...
                $next_tgt = "index.php?page=displayimage&amp;album=$album$cat_link&amp;pos=$next&amp;pid=".$next_data[0]['pid'];
                $next_title = _MD_D3XCGAL_DIS_NEXT;
        } else {
                $next_tgt = "javascript:;";
                $next_title = "";
        }
        ...
        if (USER_CAN_SEND_ECARDS){
                $ecard_tgt = "index.php?page=ecard&album=$album$cat_link&amp;pid=$pid&amp;pos=$pos";
                $ecard_title = _MD_D3XCGAL_DIS_SEND_CARD;
        } else {
                $ecard_tgt = "javascript:alert('".addslashes(_MD_D3XCGAL_DIS_CARD_DISABLEMSG)."');";
                $ecard_title = _MD_D3XCGAL_DIS_CARD_DISABLE;
        }

        $thumb_tgt = "index.php?page=thumbnails&album=$album$cat_link&amp;spage=$spage";

        $slideshow_tgt = "index.php?page=displayimage&amp;pid=$pid&amp;album=$album$cat_link&amp;slideshow=5000";

        $pic_pos = sprintf(_MD_D3XCGAL_DIS_PICPOS, $human_pos, $pic_count);

        $xoopsTpl->assign('thumb_tgt', $thumb_tgt);
        $xoopsTpl->assign('thumb_title', _MD_D3XCGAL_DIS_BACK_TNPAGE);
        $xoopsTpl->assign('pic_info_title', _MD_D3XCGAL_DIS_SHOW_PIC_INFO);
        $xoopsTpl->assign('slideshow_tgt', $slideshow_tgt);
        $xoopsTpl->assign('slideshow_title', _MD_D3XCGAL_DIS_SLIDE);
...
function html_picture()
{
    ...
    if (is_image($CURRENT_PIC_DATA['filename'])){
        ...
        if (isset($image_size['reduced'])) {
                ...
                $xoopsTpl->assign('lang_view_fs', _MD_D3XCGAL_DIS_FULL);
    ...
    }else if (is_movie($CURRENT_PIC_DATA['filename']))
    {
        ...
        if ((USER_ADMIN_MODE && $CURRENT_ALBUM_DATA['category'] == FIRST_USER_CAT + USER_ID) || GALLERY_ADMIN_MODE){
          $xoopsTpl->assign('lang_confirm_del', _MD_D3XCGAL_DIS_CONF_DEL);
          $xoopsTpl->assign('lang_del_pic', _MD_D3XCGAL_DIS_DEL_PIC);
    }
    else{
...
function html_rating_box()
{
    ...
    global $xoopsTpl;
        ...
        $votes = $CURRENT_PIC_DATA['votes'] ? sprintf(_MD_D3XCGAL_DIS_RATINGCUR, round($CURRENT_PIC_DATA['pic_rating']/2000, 1), $CURRENT_PIC_DATA['votes']) : _MD_D3XCGAL_DIS_NO_VOTE;
        $pid = $CURRENT_PIC_DATA['pid'];


        $xoopsTpl->assign('lang_rate_this_pic', _MD_D3XCGAL_DIS_RATE_THIS);
        $xoopsTpl->assign('votes', $votes);
        $xoopsTpl->assign('lang_rubbish', _MD_D3XCGAL_DIS_RUBBISH);
    $xoopsTpl->assign('lang_poor', _MD_D3XCGAL_DIS_POOR);
    $xoopsTpl->assign('lang_fair', _MD_D3XCGAL_DIS_FAIR);
    $xoopsTpl->assign('lang_good', _MD_D3XCGAL_DIS_GOOD);
    $xoopsTpl->assign('lang_excellent', _MD_D3XCGAL_DIS_EXCELLENT);
    $xoopsTpl->assign('lang_great', _MD_D3XCGAL_DIS_GREAT);
...
function html_picinfo()
{
        ...
        $info[_MD_D3XCGAL_DIS_FNAME]   = $myts->makeTboxData4Show($CURRENT_PIC_DATA['filename']);
        $info[_MD_D3XCGAL_DIS_ANAME] = '<span class="alblink"><a href="index.php?page=thumbnails&album=' . $CURRENT_PIC_DATA['aid'] . '">' . $myts->makeTboxData4Show($CURRENT_ALBUM_DATA['title']).'</a></span>';
    $user_handler =& xoops_gethandler('member');
    $submitter =& $user_handler->getUser($CURRENT_PIC_DATA['owner_id']);
    if (is_object ($submitter)) {
        $info[_MD_D3XCGAL_DIS_UPLOADER] =  '<span class="alblink"><a href="'.XOOPS_URL.'/userinfo.php?uid=' . $submitter->uid() . '">' . $submitter->uname().'</a>&nbsp;&nbsp;<a href="index.php?page=thumbnails&album=usearch&amp;suid=' . $submitter->uid() . '" title="'._MD_D3XCGAL_DIS_VIEW_MORE_BY.' ' . $submitter->uname().'"><img src="images/more.gif" align="middle" alt=""/></a></span>';
    }
        if ($CURRENT_PIC_DATA['votes'] > 0) {
                $info[sprintf(_MD_D3XCGAL_DIS_RATING, $CURRENT_PIC_DATA['votes'])] = '<img src="images/rating'.round($CURRENT_PIC_DATA['pic_rating']/2000).'.gif" align="middle" alt=""/>';
        }
    $keys = explode(' ', $myts->makeTboxData4Show($CURRENT_PIC_DATA['keywords']));
    $info[_MD_D3XCGAL_KEYS] = '<span class="alblink">';
    foreach($keys as $k){
        $info[_MD_D3XCGAL_KEYS].= "<a href=\"index.php?page=thumbnails&album=search&amp;search=".rawurlencode($k)."\">{$k}</a> ";
        }
        $info[_MD_D3XCGAL_KEYS].= '</span>';
        //$info[_MD_D3XCGAL_KEYS]   = '<span class="alblink">'.preg_replace("/(\S+)/","<a href=\"thumbnails.php?album=search&amp;search=\\1\">\\1</a>" , $myts->makeTboxData4Show($CURRENT_PIC_DATA['keywords'])).'</span>';
        ...
        $info[_MD_D3XCGAL_DIS_FSIZE]  = ($CURRENT_PIC_DATA['filesize'] > 10240 ? ($CURRENT_PIC_DATA['filesize'] >> 10).' '._MD_D3XCGAL_KB : $CURRENT_PIC_DATA['filesize'].' '._MD_D3XCGAL_BYTES);
        $info[_MD_D3XCGAL_DIS_DIMEMS] = sprintf(_MD_D3XCGAL_DIS_SIZE, $CURRENT_PIC_DATA['pwidth'], $CURRENT_PIC_DATA['pheight']);
        $info[_MD_D3XCGAL_DIS_DISPLAYED]  = sprintf(_MD_D3XCGAL_DIS_VIEWS, $CURRENT_PIC_DATA['hits']);
    $info[_MD_D3XCGAL_DIS_SENT]  = sprintf(_MD_D3XCGAL_DIS_VIEWS, $CURRENT_PIC_DATA['sent_card']);
    ...
    if (isset($exif) && is_array($exif)){
                ...
                if (isset($exif['Camera'])) $info[_MD_D3XCGAL_DIS_CAMERA] = $exif['Camera'];
                if (isset($exif['DateTaken'])) $info[_MD_D3XCGAL_DIS_DATA_TAKEN] = $exif['DateTaken'];
                if (isset($exif['Aperture'])) $info[_MD_D3XCGAL_DIS_APERTURE] = $exif['Aperture'];
                if (isset($exif['ExposureTime'])) $info[_MD_D3XCGAL_DIS_EXPTIME] = $exif['ExposureTime'];
                if (isset($exif['FocalLength']))  $info[_MD_D3XCGAL_DIS_FLENGTH] = $exif['FocalLength'];
                if (isset($exif['Comment'])) $info[_MD_D3XCGAL_DIS_COMMENT] = $exif['Comment'];
        }
        if(USER_IS_ADMIN) $info[_MD_D3XCGAL_DIS_SUBIP] = $CURRENT_PIC_DATA['ip'];
        $xoopsTpl->assign('lang_picinfo_title', _MD_D3XCGAL_DIS_TITLE);
...
{
        global $xoopsModule, $xoopsModuleConfig, $_GET, $ALBUM_SET;
        ...
        if (isset($_GET['picfile'])){
                ...
                if (!GALLERY_ADMIN_MODE) redirect_header('index.php',2,_MD_D3XCGAL_ACCESS_DENIED);
                ...
                $pid = (int)$_GET['pid'];
                $sql =  "SELECT * ".
                                "FROM ".$xoopsDB->prefix($mydirname."_pictures")." ".
                ...
                $pic_out= "<img src=\"".$pic_url."\" $geom class=\"image\" border=\"0\" alt=\"". htmlspecialchars($row['filename'], ENT_QUOTES). "\" /><br />\n";
        }
...
{
    global $ALBUM_SET_ARRAY, $xoopsModule, $xoopsDB;
    $mydirname = $xoopsModule->getVar('dirname') ;

        $result = $xoopsDB->query("SELECT cid, name, description FROM ".$xoopsDB->prefix($mydirname."_categories")." WHERE parent = '$parent'");
        if ($xoopsDB->getRowsNum($result) > 0){
                $rowset = db_fetch_rowset($result);
                foreach ($rowset as $subcat){
                        $result=$xoopsDB->query("SELECT aid FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category = {$subcat['cid']}");
...
if (!is_numeric($album) && $cat) { // Meta albums, we need to restrict the albums to the current category
        if ($cat < 0) {
            $ALBUM_SET .= 'AND aid IN ('.(- $cat).') ';
                ...
                $result=$xoopsDB->query("SELECT aid FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE $where");
                while($row = $xoopsDB->fetchArray($result)){
                        $ALBUM_SET_ARRAY[] = $row['aid'];
                } // while
...
if ($pos < 0) {
    $pid = -$pos;
        $result = $xoopsDB->query("SELECT aid from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE pid='$pid' $ALBUM_SET LIMIT 1");
        if ($xoopsDB->getRowsNum($result) == 0) redirect_header('index.php',2,_MD_D3XCGAL_NON_EXIST_AP);
        $row = $xoopsDB->fetchArray($result);
        $album = $row['aid'];
        ...
        $pic_data = get_pic_data($album, $pic_count, $album_name, $pos, 1, false);
        if ($pic_count == 0) {
            redirect_header('index.php',2,_MD_D3XCGAL_NO_IMG_TO_DISPLAY);
...
if (isset($CURRENT_PIC_DATA)) {
        $result = $xoopsDB->query("SELECT title, comments, votes, category FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE aid='{$CURRENT_PIC_DATA['aid']}' LIMIT 1");
        if (!$xoopsDB->getRowsNum($result)) redirect_header('index.php',2, sprintf(_MD_D3XCGAL_PIC_IN_INVALID_ALBUM,$CURRENT_PIC_DATA['aid']));
...
if (isset($_GET['fullsize'])){
    ...
    require_once XOOPS_ROOT_PATH.'/class/template.php';
        ...
        $xoopsTpl->display('db:'.$mydirname.'_fullsize.html');
        exit();

} elseif (isset($_GET['slideshow'])) {
    $xoopsOption['template_main'] = $mydirname.'_slideshow.html';
    ...
    $xoopsTpl->assign('lang_stop_slideshow',_MD_D3XCGAL_DIS_STOP_SLIDE);

    user_save_profile();
    $xoopsTpl->assign('gallery', $xoopsModule->getVar('name'));
    include_once dirname(dirname(__FILE__)).'/include/theme_func.php';
    main_menu();
        //$xoopsTpl->assign('d3xcgal_footer', pagefooter());
        do_footer();
        include_once XOOPS_ROOT_PATH."/footer.php";
} else {
        if (!isset($pos)) redirect_header('index.php',2,_MD_D3XCGAL_NON_EXIST_AP);
        $picture_title = $CURRENT_PIC_DATA['title'] ? $CURRENT_PIC_DATA['title'] : strtr(preg_replace("/(.+)\..*?\Z/", "\\1", htmlspecialchars($CURRENT_PIC_DATA['filename'], ENT_QUOTES)), "_", " ");
    $xoopsOption['template_main']= $mydirname.'_display.html';
    include XOOPS_ROOT_PATH."/header.php";
    $xoopsTpl->assign('mydirname',$mydirname);
    $xoopsTpl->assign('xoops_module_header', $d3xcgal_module_header);
