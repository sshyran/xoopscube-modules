...
if ( file_exists(dirname(dirname(__FILE__)).'/language/'.$xoopsConfig['language'].'/admin.php') ) {
        include_once dirname(dirname(__FILE__)).'/language/'.$xoopsConfig['language'].'/admin.php';
} else {
        include_once dirname(dirname(__FILE__)).'/language/english/admin.php';
}
include_once XOOPS_ROOT_PATH."/include/xoopscodes.php";
include_once XOOPS_ROOT_PATH.'/class/module.errorhandler.php';
$eh = new ErrorHandler;

$alb_path= XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/';
...
function albumselect($id="album")
{
        ...
        if ($select == ""){
               ...
               "FROM ".$xoopsDB->prefix($mydirname."_albums")." ".
                           "ORDER BY title";
...
function dirheader($dir, $dirid)
{
        ...
        if (!is_writable(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$dir))
                $warning ="<tr><td class=\"even\" valign=\"middle\" colspan=\"3\">\n".
                        "<div class=\"errorMsg\"><b>"._AM_D3XCGAL_SRCHNEW_WARNING."</b>: "._AM_D3XCGAL_SRCHNEW_CHG_PERM."</div></td></tr>\n";
        return "<tr><td class=\"head\" valign=\"middle\" colspan=\"3\">\n".
                        sprintf(_AM_D3XCGAL_SRCHNEW_TARGET_ALB, $dir, albumselect($dirid))."</td></tr>\n".$warning;
}
...
{
        global $xoopsModule, $xoopsModuleConfig, $expic_array;
        $mydirname = $xoopsModule->getVar('dirname') ;

        $encoded_picfile = base64_encode($picfile);
        $picname = XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$picfile;
        $pic_url = urlencode($picfile);
        $picpath = XOOPS_URL.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$picfile;
        ...
        if(file_exists($thumb_file)){
                ...
                $img ='<img src="index.php?page=showthumb&picfile='.$pic_url.'&size=48" class="thumbnail" border="0">';
        }

        if (!preg_match('/([^\/]+\.html)$/', $picname) && filesize($picname) && is_readable($picname)) {
        ...
        elseif ($xoopsModuleConfig['batch_all'] == 1){
            return <<<EOT
                   <tr>
                       ...
                       <td class="even" valign="middle" width="100%">
                           <a href="javascript:;" onClick= "MM_openBrWindow('index.php?page=displayimage&fullsize=1&picfile=$pic_url', 'ImageViewer', 'toolbar=yes, status=yes, resizable=yes, width=$winsizeX, height=$winsizeY')">$pic_fname</a>
                       </td>
                       <td class="odd" valign="middle" align="center">
                           <a href="javascript:;" onClick= "MM_openBrWindow('index.php?page=displayimage&fullsize=1&picfile=$pic_url', 'ImageViewer', 'toolbar=yes, status=yes, resizable=yes, width=$winsizeX, height=$winsizeY')">$img<br /></a>
                       </td>
                   </tr>
EOT;
        }
        ...
        <tr>
                ...
                <td class="odd" valign="middle" align="center">
                        <a href="javascript:;" onClick= "MM_openBrWindow('index.php?page=displayimage&fullsize=1&picfile=$pic_url', 'ImageViewer', 'toolbar=yes, status=yes, resizable=yes, width=$winsizeX, height=$winsizeY')"><img src="index.php?page=showthumb&picfile=$pic_url&size=48" class="thumbnail" border="0"><br /></a>
                </td>
        </tr>
EOT;
        }
...
function getfoldercontent($folder, &$dir_array, &$pic_array, &$expic_array)
{
        ...
        $dir = opendir(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$folder);
        while($file = readdir($dir)){
                if(is_dir(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$folder.$file)) {
                        if ($file != "." && $file != "..")
                                $dir_array[] = $file;
                }
                if(is_file(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$folder.$file)) {
...
{
        global $xoopsModule, $xoopsModuleConfig, $PHP_SELF;
        $mydirname = $xoopsModule->getVar('dirname') ;

        $dir_path = XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$folder;
        if (!is_readable($dir_path)) return;

        $dir = opendir($dir_path);
        while($file = readdir($dir)){
                if(is_dir(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$folder.$file) && $file != "." && $file != "..") {
                        $start_target = $folder.$file;
                        $dir_path = XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'].$folder.$file;

                        $warnings = '';
                        if (!is_writable($dir_path)) $warnings .= _AM_D3XCGAL_SRCHNEW_DIR_RO;
                        if (!is_readable($dir_path)) $warnings .= _AM_D3XCGAL_SRCHNEW_CANT_READ;
                        ...
                        <tr>
                                <td class="even">
                                        $ident<img src="images/folder.gif" alt="">&nbsp;<a href= "index.php?page=batchupload&startdir=$start_target">$file</a>$warnings
                                </td>
                        </tr>
EOT;
...
if (!count($album_array)) redirect_header('index.php',2,_AM_D3XCGAL_SRCHNEW_NEED_ONE_ALB);

if (isset($_POST['insert'])){

        if(!isset($_POST['pics'])) redirect_header('index.php',2,_AM_D3XCGAL_SRCHNEW_NO_PIC_ADD);

        include XOOPS_ROOT_PATH."/header.php";

        echo "<table border='0' cellpadding='0' cellspacing='1' width='100%' class='outer'>";
        echo "<tr><th colspan='4'>"._AM_D3XCGAL_SRCHNEW_TITLE."</th></tr>";
        echo "<tr><td colspan='4' class='head'>"._AM_D3XCGAL_SRCHNEW_INSERT."</td></tr>";
        echo "<tr>";
        echo "<td class='head' valign='middle' align='center'><b>"._AM_D3XCGAL_SRCHNEW_FOLDER."</b></td>";
        echo "<td class='head' valign='middle' align='center'><b>"._AM_D3XCGAL_SRCHNEW_IMAGE."</b></td>";
    echo "<td class='head' valign='middle' align='center'><b>"._AM_D3XCGAL_SRCHNEW_ALB."</b></td>";
    echo "<td class='head' valign='middle' align='center'><b>"._AM_D3XCGAL_SRCHNEW_RESULT."</b></td>";
        echo "</tr>";
        ...
        foreach ($_POST['pics'] as $pic_id){
                ...
                $status = "<a href=\"index.php?page=addpic&aid=$album_id&pic_file=".($_POST['picfile_'.$pic_id])."&reload=".uniqid('')."\"><img src=\"index.php?page=addpic&aid=$album_id&pic_file=".($_POST['picfile_'.$pic_id])."&reload=".uniqid('')."\" class=\"thumbnail\" border=\"0\" width=\"24\" height=\"24\" /><br /></a>";
        ...
        echo "<tr><td class='foot' colspan='4'><b>"._AM_D3XCGAL_SRCHNEW_PATIENT."</b></td></tr>";
        echo "<tr><td class='foot' colspan='4'>"._AM_D3XCGAL_SRCHNEW_NOTES."</td></tr>";
} elseif(isset($_GET['startdir'])){

       include XOOPS_ROOT_PATH."/header.php";

        echo "<table border='0' cellpadding='0' cellspacing='1' width='100%' class='outer'>";
        echo "<tr><th colspan='3'>"._AM_D3XCGAL_SRCHNEW_TITLE."</th></tr>";
        echo "<form method=\"post\" action=\"index.php?page=batchupload&insert=1\">";	//change @2007/11/12
        echo "<tr><td colspan='3' class='head'>"._AM_D3XCGAL_SRCHNEW_LIST_NEW."</td></tr>";
        $expic_array = array();
        ...
        if (xcscandir($_GET['startdir'].'/', $expic_array)){
            ...
            echo "<input type=\"submit\" class=\"button\" name=\"insert\" value=\""._AM_D3XCGAL_SRCHNEW_INS_SEL."\" />";
                echo "</td></tr></form>";
        } else {
            echo "<tr><td colspan='3' align='center' class='even'>";
            echo "<div class='errorMsg'><br /><br />";
            echo "<b>"._AM_D3XCGAL_SRCHNEW_NO_PIC."</b>";
      ...
      include XOOPS_ROOT_PATH."/header.php";
        ...
        echo "<table border='0' cellpadding='0' cellspacing='1' width='100%' class='outer'>";
        echo "<tr><th>"._AM_D3XCGAL_SRCHNEW_TITLE."</th></tr>";
        display_dir_tree('','');
        echo "<tr><td class='foot'><b>"._AM_D3XCGAL_SRCHNEW_SEL_DIR_MSG."</b></td></tr>";
}
