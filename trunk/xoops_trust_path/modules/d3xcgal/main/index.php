...
{
        $amenu['conf_del'] = _MD_D3XCGAL_INDEX_CONF_DEL;
        $amenu['delete']= _MD_D3XCGAL_INDEX_DEL;
        $amenu['modify']= _MD_D3XCGAL_INDEX_MOD;
        $amenu['edit_pics']= _MD_D3XCGAL_INDEX_EDIT;
...
{
    global $xoopsModule, $xoopsModuleConfig, $HIDE_USER_CAT, $xoopsDB, $myts;

        $mydirname = $xoopsModule->getVar('dirname') ;
        $sql = $xoopsDB->query("SELECT cid, name, description FROM ".$xoopsDB->prefix($mydirname."_categories")." WHERE parent = '$parent'  ORDER BY pos");
        ...
        if ($xoopsDB->getRowsNum($sql) > 0){
                ...
                while($subcat = $xoopsDB->fetchArray($sql)) {
                        ...
                        if ($subcat['cid'] == USER_GAL_CAT) {
                                $result= $xoopsDB->query("SELECT aid FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category >= ".FIRST_USER_CAT);
                                ...
                                $result= $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_pictures").", ".$xoopsDB->prefix($mydirname."_albums")." WHERE ".$xoopsDB->prefix($mydirname."_pictures").".aid = ".$xoopsDB->prefix($mydirname."_albums").".aid AND category >= ".FIRST_USER_CAT);
                ...
                $xoopsDB->freeRecordSet($result);
                                ...
                                $result= $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_pictures").", ".$xoopsDB->prefix($mydirname."_albums")." WHERE ".$xoopsDB->prefix($mydirname."_pictures").".approved='YES' AND ".$xoopsDB->prefix($mydirname."_pictures").".aid = ".$xoopsDB->prefix($mydirname."_albums").".aid AND category = {$subcat['cid']}");
                                $pic_count = $xoopsDB->getRowsNum($result);
                $xoopsDB->freeRecordSet($result);
...
function get_cat_list(&$breadcrumb, &$cat_data, &$statistics)
{
        ...
        if ($cat) {
                if ($cat == USER_GAL_CAT) {
                        ...
                        $result=$xoopsDB->query("SELECT aid FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category = '$cat'");
                }
                while($row = $xoopsDB->fetchArray($result)){
                        $album_set_array[] = $row['aid'];
                } // while
        ...
        if ($cat == 0) {
                $result=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE 1");
                $album_count = $xoopsDB->getRowsNum($result);
                $xoopsDB->freeRecordSet($result);

                $result=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE 1 AND approved='YES'");
                $picture_count = $xoopsDB->getRowsNum($result);
                $xoopsDB->freeRecordSet($result);

                $comment_count = xoops_comment_count($xoopsModule->mid());

                $result=$xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_categories")." WHERE 1");
                $cat_count = $xoopsDB->getRowsNum($result) - $HIDE_USER_CAT;
                $xoopsDB->freeRecordSet($result);

                $result=$xoopsDB->query("SELECT sum(hits) FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE 1");
                $nbEnr = $xoopsDB->fetchArray($result);
                $hit_count = (int)$nbEnr['sum(hits)'];
                $xoopsDB->freeRecordSet($result);

                if (count($cat_data)) {
                        $statistics = strtr(_MD_D3XCGAL_INDEX_STAT1, array(
                        ...
                        $statistics = strtr(_MD_D3XCGAL_INDEX_STAT3, array(
                                '[pictures]' => $picture_count,
                                '[albums]' => $album_count,
                                '[comments]' => $comment_count,
                                '[views]' => $hit_count));
                ...
                $result=$xoopsDB->query("SELECT count(*) FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE 1 $current_album_set");
                $nbEnr = $xoopsDB->fetchArray($result);
                $album_count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);

                $result=$xoopsDB->query("SELECT count(*) FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE 1 $current_album_set");
                $nbEnr = $xoopsDB->fetchArray($result);
                $picture_count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);

                $result=$xoopsDB->query("SELECT sum(hits) FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE 1 $current_album_set");
                $nbEnr = $xoopsDB->fetchArray($result);
                $hit_count = (int)$nbEnr['sum(hits)'];
                $xoopsDB->freeRecordSet($result);

                $statistics = strtr(_MD_D3XCGAL_INDEX_STAT2, array(
                                '[pictures]' => $picture_count,
                                '[albums]' => $album_count,
                                '[views]' => $hit_count));
...
function list_users()
{
        global $xoopsModule, $xoopsModuleConfig, $SPAGE, $FORBIDDEN_SET, $xoopsDB, $myts;
        ...
        $sql =  "SELECT category, COUNT(DISTINCT a.aid) as alb_count,".
                        ...
                        "FROM ".$xoopsDB->prefix($mydirname."_albums")." AS a, ".
                        "".$xoopsDB->prefix($mydirname."_pictures")." AS p WHERE category > ".FIRST_USER_CAT." AND p.aid = a.aid ".
                        "AND approved = 'YES' ".
                        "$FORBIDDEN_SET ".
                        "GROUP BY category ".
                        "ORDER BY category ";
        ...
        if (!$user_count) {
                   redirect_header('index.php',2,_MD_D3XCGAL_INDEX_NO_UGAL);
        ...
        $lower_limit = ($SPAGE-1) * $user_per_page;
        $upper_limit = min($user_count, $SPAGE * $user_per_page);
        ...
        for ($i = $lower_limit; $i < $upper_limit; $i++){
                ...
                $albums_txt = sprintf(_MD_D3XCGAL_INDEX_NALBS, $user_album_count);
                $pictures_txt = sprintf(_MD_D3XCGAL_NPICS, $user_pic_count);
        ...
        theme_display_thumbnails($user_list, $user_count, '', '', 1, $SPAGE, $totalPages, false, true, 'user');
}
...
{
        global $xoopsModule, $xoopsModuleConfig, $USER, $SPAGE, $lastup_date_fmt, $_GET, $USER_DATA;
        global $xoopsDB;
    ...
    $myts =& MyTextSanitizer::getInstance(); // MyTextSanitizer object
        ...
        $result = $xoopsDB->query("SELECT count(*) FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE category = '$cat'");
        $nbEnr = $xoopsDB->fetchArray($result);
        $nbAlb = $nbEnr['count(*)'];
        $xoopsDB->freeRecordSet($result);
        ...
        if ($SPAGE > $totalPages) $SPAGE = 1;
        $lower_limit = ($SPAGE-1) * $alb_per_page;
        $upper_limit = min($nbAlb, $SPAGE * $alb_per_page);
        $limit = "LIMIT ". $lower_limit . "," . ($upper_limit-$lower_limit);

        $sql =  "SELECT a.aid, a.title, description, visibility, filepath, ".
                        "                filename, url_prefix, pwidth, pheight ".
                        "FROM ".$xoopsDB->prefix($mydirname."_albums")." as a ".
                        "LEFT JOIN ".$xoopsDB->prefix($mydirname."_pictures")." as p ON thumb=pid ".
        ...
        for ($alb_idx=0; $alb_idx < $disp_album_count; $alb_idx++) {
                ...
                $alb_list[$alb_idx]['album_info']     = sprintf(_MD_D3XCGAL_NPICS, $count);
    ...
    theme_display_album_list($alb_list, $nbAlb, $cat, $SPAGE, $totalPages);
}
...
if (isset($_GET['spage'])){
        $SPAGE = max((int)$_GET['spage'], 1);
        $USER['lap'] = $SPAGE;
} elseif (isset($USER['lap'])){
        $SPAGE = max((int)$USER['lap'],1);
} else {
        $SPAGE = 1;
}
...
//$xoopsTpl->assign('d3xcgal_main', $temp);
...
$result=$xoopsDB->query("SELECT count(*) FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE 1");
$nbEnr = $xoopsDB->fetchArray($result);
$xoopsDB->freeRecordSet($result);
...
if ($granularity != RANDPOS_INTERVAL && $pic_count > RANDPOS_MAX_PIC) {
    $result=$xoopsDB->queryf("UPDATE ".$xoopsDB->prefix($mydirname."_pictures")." SET randpos = ROUND(RAND()*$granularity) WHERE 1");
    $result=$xoopsDB->queryf("UPDATE ".$xoopsDB->prefix($mydirname."_config")." SET value = '$granularity' WHERE name = 'randpos_interval'");
}
