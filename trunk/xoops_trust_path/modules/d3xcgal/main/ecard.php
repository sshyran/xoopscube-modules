...
if (!USER_CAN_SEND_ECARDS) redirect_header('index.php',2,_MD_D3XCGAL_ACCESS_DENIED);
function send_ecard($recipient_email, $recipient_name, $greetings, $msg_content, $sender_name, $sender_email, $image, $n_picname, $redirect_link)
{
    ...
    $s_time = time()-3600;
    $result = $xoopsDB->query("SELECT * from ".$xoopsDB->prefix($mydirname."_ecard")." WHERE (sess_id ='".session_id()."' || sender_email = '".$myts->makeTboxData4Save($sender_email)."' || sender_ip ='".$_SERVER['REMOTE_ADDR']."' ".$s_uid.") AND s_time > ".$s_time."");
    if ($xoopsDB->getRowsNum($result) >= $xoopsModuleConfig['ecards_per_hour']){
        redirect_header('index.php',2,sprintf(_MD_D3XCGAL_CARD_PERHOUR, $xoopsModuleConfig['ecards_per_hour']));
        return;
        }
    ...
    $ecardText= str_replace("{CARD_LINK}", XOOPS_URL.'/modules/'.$mydirname.'/index.php?page=displayecard&data='.$e_id , $ecardText);    $xoopsMailer->setToEmails($recipient_email);


    $xoopsMailer->setSubject(sprintf(_MD_D3XCGAL_CARD_ECARD_TITLE, $sender_name));
    ...
    $xoopsMailer->send(true);
        ...
        if (!empty($ev_success)){
                redirect_header($redirect_link,2,_MD_D3XCGAL_CARD_SEND_SUCCESS);
                exit;
        } else {
                redirect_header($redirect_link,2,_MD_D3XCGAL_CARD_SEND_FAILED."<br />".$ev_errors);
                exit;
        }
...
function build_html_card($sender_name, $sender_email, $n_picname, $message, $greetings,$e_id){
    global $myts, $xoopsConfig, $xoopsModuleConfig, $xoopsModule;
    $mydirname = $xoopsModule->getVar('dirname') ;
        if(!stristr($n_picname, 'http:')) $n_picname = XOOPS_URL.'/modules/'.$mydirname.'/'.$n_picname;
    ...
    require_once XOOPS_ROOT_PATH.'/class/template.php';
        ...
        $ecardTpl->assign('ecard_title', sprintf(_MD_D3XCGAL_CARD_ECARD_TITLE, $myts->makeTboxData4Show($sender_name)));
        $ecardTpl->assign('charset',_CHARSET);
        $ecardTpl->assign('view_ecard_tgt', XOOPS_URL.'/modules/'.$mydirname.'/index.php?page=displayecard&data='.$e_id);
        $ecardTpl->assign('view_ecard_lnk',_MD_D3XCGAL_CARD_VIEW_ECARD);
        ...
        $ecardTpl->assign('view_more_lnk', _MD_D3XCGAL_CARD_VIEW_MORE_PICS);
        $ecardTpl->assign('xoops_module_header', $d3xcgal_module_header);
        $ecardTpl->assign('xoops_url', XOOPS_URL);
    $card = $ecardTpl->fetch('db:d3xcgal_discard.html');
    return $card;
...
$result = $xoopsDB->query("SELECT * from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE pid='$pid' $ALBUM_SET");
...
$invalid_email = '<font size="1">'._MD_D3XCGAL_CARD_INVALIDE_EMAIL.'</font>';
...
if (count($_POST) > 0 && $valid_sender_email && $valid_recipient_email){
        ...
        $redirect_link = "index.php?page=displayimage&pid=".$row['pid']."&amp;album=".$album."&amp;pos=".$pos."";
...
$xoopsOption['template_main'] = $mydirname.'_ecard.html';
include XOOPS_ROOT_PATH."/header.php";
$xoopsTpl->assign('mydirname',$mydirname);
$xoopsTpl->assign('xoops_module_header', $d3xcgal_module_header);
$xoopsTpl->assign('ecard_title',_MD_D3XCGAL_CARD_TITLE);
$xoopsTpl->assign('lang_ecard_from',_MD_D3XCGAL_CARD_FROM);
$xoopsTpl->assign('thumb_url',$thumb_pic_url);
$xoopsTpl->assign('album',$album);
$xoopsTpl->assign('pid',$pid);
$xoopsTpl->assign('pos',$pos);
$xoopsTpl->assign('lang_your_name',_MD_D3XCGAL_CARD_YOUR_NAME);
$xoopsTpl->assign('sender_name',$sender_name);
$xoopsTpl->assign('lang_your_email',_MD_D3XCGAL_CARD_YOUR_EMAIL);
$xoopsTpl->assign('sender_email',$sender_email);
$xoopsTpl->assign('sender_email_warning',$sender_email_warning);
$xoopsTpl->assign('lang_ecard_to',_MD_D3XCGAL_CARD_TO);
$xoopsTpl->assign('lang_rcpt_name',_MD_D3XCGAL_CARD_RCPT_NAME);
$xoopsTpl->assign('recipient_name',$recipient_name);
$xoopsTpl->assign('lang_rcpt_email',_MD_D3XCGAL_CARD_RCPT_EMAIL);
$xoopsTpl->assign('recipient_email',$recipient_email);
$xoopsTpl->assign('recipient_email_warning',$recipient_email_warning);
$xoopsTpl->assign('lang_ecard_greetings',_MD_D3XCGAL_CARD_GREETINGS);
$xoopsTpl->assign('greetings',$greetings);
$xoopsTpl->assign('lang_ecard_message',_MD_D3XCGAL_CARD_MESSAGE);
