...
function add_picture($aid, $filepath, $filename, $title='', $caption='', $keywords='', $user1='', $user2='', $user3='', $user4='', $category=0)
{
        ...
        global $xoopsModule, $xoopsDB, $xoopsUser, $picinID, $_SERVER;
    ...
    $myts =& MyTextSanitizer::getInstance();// MyTextSanitizer object
        ...
        if(!$imagesize)
        {
            if(is_movie($image))
            {
                ...
                {
                    copy(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/images/thumb_avi.jpg', $thumb);
                    copy(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/images/thumb_avi.jpg', $normal);
                }
                   $imagesize[0]=320;
                   $imagesize[1]=240;
            }
            ...
            elseif(is_audio($image))
            {
                ...
                {
                    copy(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/images/thumb_mp3.jpg', $thumb);
                    copy(XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/images/thumb_mp3.jpg', $normal);
                }
                   $imagesize[0]=320;
                   $imagesize[1]=240;
            }
        ...
        if (!USER_IS_ADMIN && $USER_DATA['group_quota']) {
            ...
            $result =$xoopsDB->query("SELECT sum(total_filesize) FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE $quota_opt");
                ...
                if ($total_space_used + $total_filesize > ($USER_DATA['group_quota'] << 10)){
                        ...
                        @unlink($normal);
                        @unlink($thumb);
                        $msg = strtr(_MD_D3XCGAL_QUOTA_EXCEEDED, array(
...
function resize_image($src_file, $dest_file, $new_size, $method)
{
        ...
        if ($imginfo[2] != GIS_GIF && $imginfo[2] != GIS_JPG && $imginfo[2] != GIS_PNG && ($method == 'gd1' || $method == 'gd2')){
                $ERROR = _MD_D3XCGAL_GD_FILE_TYPE_ERR;
                return false;
        }
        ...
        case "im" :
                ...
                if ($retval) {
                    $ERROR = _MD_D3XCGAL_IM_ERROR." $retval";
                        if ($xoopsModuleConfig['debug_mode']) {
                                ...
                                $output = `$cmd 2>&1`;
                            $ERROR .= "<br /><br /><div align=\"left\">"._MD_D3XCGAL_IM_ERROR_CMD."<br /><font size=\"2\">".nl2br(htmlspecialchars($cmd, ENT_QUOTES))."</font></div>";
                            $ERROR .= "<br /><br /><div align=\"left\">"._MD_D3XCGAL_IM_ERROR_CONV."<br /><font size=\"2\">";
                                $ERROR .= nl2br(htmlspecialchars($output, ENT_QUOTES));
                                $ERROR .= "</font></div>";
                        }
                        @unlink($dest_file);
                        return false;
    ...
    case "net" :
        ...
        case "gd1" :
                if (!function_exists('imagecreatefromjpeg')) {
                    redirect_header('index.php',2,_MD_D3XCGAL_NO_GD_FOUND);
                }
        ...
        case "gd2" :
                ...
                if (!function_exists('imagecreatetruecolor')) {
                    redirect_header('index.php',2,_MD_D3XCGAL_GD_VERSION_ERR);
                }
                if ($imginfo[2] == GIS_JPG)
                        $src_img = imagecreatefromjpeg($src_file);
                elseif ($imginfo[2] == GIS_GIF)
        ...
        if ($imginfo == null){
                $ERROR = _MD_D3XCGAL_RESIZE_FAILED;
                @unlink($dest_file);
                return false;
        } else {
                return true;
