...
function main_menu(){
    global $album, $actual_cat, $cat, $REFERER, $_SERVER;
        ...
        if (GALLERY_ADMIN_MODE || USER_ADMIN_MODE) {
            ...
            $xoopsTpl->assign('albmgr_lnk',_MD_D3XCGAL_THM_ALBMGR_LNK);
            $xoopsTpl->assign('modifyalb_lnk',_MD_D3XCGAL_THM_MODIFY_LNK);
    ...
    $xoopsTpl->assign('alb_list_title',_MD_D3XCGAL_THM_ALB_LT);
    $xoopsTpl->assign('alb_list_lnk',_MD_D3XCGAL_THM_ALB_LL);
    $xoopsTpl->assign('my_gallery_id',$my_gallery_id);
    $xoopsTpl->assign('my_gal_title',_MD_D3XCGAL_THM_GAL_MYT);
    $xoopsTpl->assign('my_gal_lnk',_MD_D3XCGAL_THM_GAL_MYL);
    $xoopsTpl->assign('referer',$REFERER);
    $xoopsTpl->assign('adm_mode_title',_MD_D3XCGAL_THM_ADM_MT);
    $xoopsTpl->assign('adm_mode_lnk',_MD_D3XCGAL_THM_ADM_ML);
    $xoopsTpl->assign('usr_mode_title',_MD_D3XCGAL_THM_USER_MT);
    $xoopsTpl->assign('usr_mode_lnk',_MD_D3XCGAL_THM_USER_ML);
    $xoopsTpl->assign('upload_pic_title',_MD_D3XCGAL_THM_UPLT);
    $xoopsTpl->assign('upload_pic_lnk',_MD_D3XCGAL_THM_UPLL);
    $xoopsTpl->assign('uploadmore_pic_title',_MD_D3XCGAL_THM_UPLTMORE);
    $xoopsTpl->assign('uploadmore_pic_lnk',_MD_D3XCGAL_THM_UPLLMORE);
    $xoopsTpl->assign('uploadbatch_title',_MD_D3XCGAL_THM_UPLTBATCH);
    $xoopsTpl->assign('uploadbatch_lnk',_MD_D3XCGAL_THM_UPLLBATCH);
    $xoopsTpl->assign('cat_l2',$cat_l2);
    $xoopsTpl->assign('lastup_lnk',_MD_D3XCGAL_THM_LAST_UPL);
    $xoopsTpl->assign('lastcom_lnk',_MD_D3XCGAL_THM_LAST_COM);
    $xoopsTpl->assign('topn_lnk',_MD_D3XCGAL_THM_MOST_VIEW);
    $xoopsTpl->assign('toprated_lnk',_MD_D3XCGAL_THM_TOP_RATE);
    $xoopsTpl->assign('search_lnk',_MD_D3XCGAL_THM_SEARCH);
...
function theme_display_cat_list($breadcrumb, &$cat_data, $statistics)
{
    ...
    if (!isset($breadcrumb) || $breadcrumb== '') $breadcrumb = '<a href="index.php">'.$xoopsModule->getVar('name').'</a>';
        ...
        if (count($cat_data)>0) {
                $xoopsTpl->assign('lang_category',_MD_D3XCGAL_THM_CAT);
                $xoopsTpl->assign('lang_albums',_MD_D3XCGAL_THM_ALB);
                $xoopsTpl->assign('lang_pictures',_MD_D3XCGAL_THM_PIC);
...
function theme_display_album_list(&$alb_list,$nbAlb, $cat, $spage, $total_pages)
{
    ...
    $xoopsTpl->assign('display_alb_list',1);
        ...
        $theme_alb_list_tab_tmpl['left_text'] = strtr($theme_alb_list_tab_tmpl['left_text'], array('{LEFT_TEXT}' => _MD_D3XCGAL_THM_ALBONPAGE));
        $theme_alb_list_tab_tmpl['inactive_tab'] = strtr($theme_alb_list_tab_tmpl['inactive_tab'], array('{LINK}' => 'index.php?cat='.$cat.'&amp;spage=%d'));

        $tabs = create_tabs($nbAlb, $spage, $total_pages, $theme_alb_list_tab_tmpl);
        $count = 0;
...
function theme_display_thumbnails(&$thumb_list, $nbThumb, $album_name, $aid, $cat, $spage, $total_pages, $sort_options, $display_tabs, $mode='thumb')
{
        ...
        if ($mode == 'thumb') {
                $theme_thumb_tab_tmpl['left_text'] = strtr($theme_thumb_tab_tmpl['left_text'], array('{LEFT_TEXT}' => _MD_D3XCGAL_THM_PICPAGE));
                $theme_thumb_tab_tmpl['inactive_tab'] = strtr($theme_thumb_tab_tmpl['inactive_tab'], array('{LINK}' => 'index.php?page=thumbnails&album='.$aid.$cat_link.'&amp;spage=%d'));
        } else {
                $theme_thumb_tab_tmpl['left_text'] = strtr($theme_thumb_tab_tmpl['left_text'], array('{LEFT_TEXT}' => _MD_D3XCGAL_THM_USERPAGE));
                $theme_thumb_tab_tmpl['inactive_tab'] = strtr($theme_thumb_tab_tmpl['inactive_tab'], array('{LINK}' => 'index.php?cat='.$cat.'&amp;spage=%d'));
        }

        $thumbcols = $xoopsModuleConfig['thumbcols'];
        $cell_width = ceil(100/$xoopsModuleConfig['thumbcols']).'%';

        $tabs_html = $display_tabs ? create_tabs($nbThumb, $spage, $total_pages, $theme_thumb_tab_tmpl) : '';
        $i = 0;
        foreach($thumb_list as $thumb){
            ...
            $pic['user']=0;
                ...
                if ($mode == 'thumb') {
                    $pic['link_tgt']="index.php?page=displayimage&pid={$thumb['pid']}&amp;album=$aid$cat_link&amp;pos={$thumb['pos']}";
        ...
        $xoopsTpl->append('thumbs',array('mode'=>$mode,'sort_options'=>$sort_options,'album_name'=>$album_name,'aid'=>$aid,'spage'=>$spage,'name'=>_MD_D3XCGAL_THM_NAME,'date'=>_MD_D3XCGAL_THM_DATE,
                                     'sort_na'=>_MD_D3XCGAL_THM_SORT_NA,'sort_nd'=>_MD_D3XCGAL_THM_SORT_ND,'sort_da'=>_MD_D3XCGAL_THM_SORT_DA,'sort_dd'=>_MD_D3XCGAL_THM_SORT_DD,
