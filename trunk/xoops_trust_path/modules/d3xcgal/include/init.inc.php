...
$d3xcgal_module_header = "<link rel=\"stylesheet\" type=\"text/css\" media=\"all\" href=\"".str_replace('{mod_url}',XOOPS_URL.'/modules/'.$mydirname,$xoopsModuleConfig['css_uri'])."\" />";
...
require XOOPS_TRUST_PATH."/modules/d3xcgal/include/functions.inc.php";	//ROOT to TRUST
...
if (is_object($xoopsUser)){
    ...
    $sql = "SELECT * FROM ".$xoopsDB->prefix($mydirname."_usergroups")." WHERE xgroupid IN ({$usergroup})";
    ...
    $results = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix($mydirname."_usergroups")." WHERE group_id = ".XOOPS_GROUP_ANONYMOUS."");
