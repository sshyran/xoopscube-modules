...
function create_tabs($items, $curr_page, $total_pages, $template)
{
        ...
        for ($spage = $start ; $spage <= $end; $spage++) {
                if ($spage == $curr_page) {
                        $tabs .= sprintf($template['active_tab'], $spage);
                } else {
                        $tabs .= sprintf($template['inactive_tab'], $spage, $spage);
                }
        }
        if ($total_pages > 1){
...
{
        global $ALBUM_SET, $USER_DATA, $FORBIDDEN_SET, $xoopsModule, $xoopsDB,$xoopsUser,$suid;
        ...
        $result = $xoopsDB->query("SELECT aid FROM ".$xoopsDB->prefix($mydirname."_albums")." WHERE visibility NOT IN ($usergroup, 0,".(FIRST_USER_CAT + USER_ID).")");
...
{
        global $USER, $xoopsModule, $xoopsModuleConfig, $ALBUM_SET, $CURRENT_CAT_NAME, $HTML_SUBST, $THEME_DIR;
        global $GLOBALS;
    ...
    $sort_code = isset($USER['sort'])? $USER['sort'] : $xoopsModuleConfig['default_sort_order'];
        ...
        if ((is_numeric($album))) {
            $album_name = get_album_name($album);
                ...
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE aid='$album' $approved $ALBUM_SET");
                ...
                $result = $xoopsDB->query("SELECT $select_columns from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE aid='$album' $approved $ALBUM_SET ORDER BY $sort_order $limit");
                ...
                if ($set_caption) foreach ($rowset as $key => $row){
                        ...
                        if ($xoopsModuleConfig['display_comment_count']) {
                                ...
                                if ($comments_nr > 0) $caption .= "<span class=\"thumb_num_comments\">".sprintf(_MD_D3XCGAL_FUNC_COM, $comments_nr )."</span>";
                        }
        ...
        case 'lastcom': // Last comments
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = $album_name = _MD_D3XCGAL_LASTCOM.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_LASTCOM;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix("xoopscomments").", ".$xoopsDB->prefix($mydirname."_pictures")." WHERE com_modid = ".$xoopsModule->mid()." AND approved='YES' AND com_itemid = pid $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                        $count = $nbEnr['count(*)'];
                        $xoopsDB->freeRecordSet($result);
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix("xoopscomments").", ".$xoopsDB->prefix($mydirname."_pictures")." WHERE com_modid = ".$xoopsModule->mid()." AND approved = 'YES' AND pid = com_itemid AND com_status=".XOOPS_COMMENT_ACTIVE." $ALBUM_SET ORDER by com_id DESC $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);
        ...
        case 'lastup': // Last uploads
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_LASTUP.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_LASTUP;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                $count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' $ALBUM_SET ORDER BY pid DESC $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);
                if ($set_caption) foreach ($rowset as $key => $row){
        ...
        case 'topn': // Most viewed pictures
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_TOPN.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_TOPN;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND hits > 0  $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                $count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND hits > 0 $ALBUM_SET ORDER BY hits DESC, ctime, mtime $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);

                if ($set_caption) foreach ($rowset as $key => $row){
                        $caption = "<span class=\"thumb_caption\">".sprintf(_MD_D3XCGAL_FUNC_VIEW, $row['hits']).'</span>';
                        $rowset[$key]['caption_text'] = $caption;
                }
        ...
        case 'toprated': // Top rated pictures
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_TOPRATED.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_TOPRATED;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND votes >= '{$xoopsModuleConfig['min_votes_for_rating']}' $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                $count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND votes >= '{$xoopsModuleConfig['min_votes_for_rating']}' $ALBUM_SET ORDER BY ROUND((pic_rating+1)/2000) DESC, votes DESC $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);
                ...
                if ($set_caption) foreach ($rowset as $key => $row){
                        ...
                        $caption = "<span class=\"thumb_caption\">".'<img src="'.$prefix.'images/rating'.round($row['pic_rating']/2000).'.gif" align="middle" alt=""/>'.'<br />'.sprintf(_MD_D3XCGAL_FUNC_VOTE, $row['votes']).'</span>';
                        $rowset[$key]['caption_text'] = $caption;
                }
                return $rowset;
                break;
        ...
        case 'lasthits': // Last viewed pictures
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_LASTHITS.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_LASTHITS;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                $count = $nbEnr['count(*)'];
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' $ALBUM_SET ORDER BY mtime DESC $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);
        ...
        case 'random': // Random pictures
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_RANDOM.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_RANDOM;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                ...
                if ($pic_count > 1000) {
                    $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES'");
                        ...
                        $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE  randpos IN ($random_num_set) AND approved = 'YES' $ALBUM_SET ORDER BY RAND() LIMIT $limit2");
                } else {
                        $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' $ALBUM_SET ORDER BY RAND() LIMIT $limit2");
                }
        ...
        case 'search': // Search results
                ...
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_SEARCH.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_SEARCH.' - "'.$search_string. '"';
                }
                //var_dump(htmlspecialchars($search_string));
        //$search_string = utf8Encode($search_string);
                //include 'include/search.inc.php';
                //include dirname(__FILE__).'/include/search.inc.php' ;	//NG @2007/11/14
                include dirname(__FILE__).'/search.inc.php' ;	//change @2007/11/14
        ...
        case 'usearch': // User pics search results
            ...
            $album_name = _MD_D3XCGAL_USEARCH.$owner->uname();
        }
        else $album_name = 'Pics submitted by '.$xoopsConfig['anonymous'];
        $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND owner_id = '{$USER['suid']}' $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                $count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND owner_id = '{$USER['suid']}' $ALBUM_SET ORDER BY ctime DESC $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);
        ...
        case 'mostsend': // Top rated pictures
                if ($ALBUM_SET && $CURRENT_CAT_NAME) {
                        $album_name = _MD_D3XCGAL_MOST_SENT.' - '. $CURRENT_CAT_NAME;
                } else {
                        $album_name = _MD_D3XCGAL_MOST_SENT;
                }
                $result = $xoopsDB->query("SELECT count(*) from ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND sent_card > 0 $ALBUM_SET");
                $nbEnr = $xoopsDB->fetchArray($result);
                $count = $nbEnr['count(*)'];
                $xoopsDB->freeRecordSet($result);
                ...
                $result = $xoopsDB->query("SELECT $select_columns FROM ".$xoopsDB->prefix($mydirname."_pictures")." WHERE approved = 'YES' AND sent_card >0 $ALBUM_SET ORDER BY sent_card DESC $limit");
                $rowset = db_fetch_rowset($result);
                $xoopsDB->freeRecordSet($result);

                if ($set_caption) foreach ($rowset as $key => $row){
                        $caption = "<span class=\"thumb_caption\">".sprintf(_MD_D3XCGAL_FUNC_SEND, $row['sent_card']).'</span>';
                        $rowset[$key]['caption_text'] = $caption;
                }
                return $rowset;
                break;
        ...
        default : // Invalid meta album
                redirect_header('index.php',2, _MD_D3XCGAL_NON_EXIST_AP);
        }
} // End of get_pic_data
...
function get_album_name($aid)
{
        ...
        $result = $xoopsDB->query("SELECT title from ".$xoopsDB->prefix($mydirname."_albums")." WHERE aid='$aid'");
...
function add_hit($pid)
{
        global $xoopsModule, $xoopsDB;
        $mydirname = $xoopsModule->getVar('dirname') ;
        $xoopsDB->queryf("UPDATE ".$xoopsDB->prefix($mydirname."_pictures")." SET hits=hits+1, mtime=".time()." WHERE pid='$pid'");
...
function breadcrumb($cat, &$breadcrumb, &$BREADCRUMB_TEXT)
{
    ...
    $myts =& MyTextSanitizer::getInstance(); // MyTextSanitizer object
        ...
        if ($cat != 0) {
            ...
            if (!is_object($alb_owner) && !USER_IS_ADMIN) redirect_header('index.php',2,_MD_D3XCGAL_NO_EXIST_CAT);
            elseif (!is_object($alb_owner)) $row['uname'] = _MD_D3XCGAL_FUNC_DELUSER." uid=".($cat - FIRST_USER_CAT);
                        else $row['uname'] = $alb_owner->uname();
                        $breadcrumb_array[] = array($cat, $row['uname']);
                        $CURRENT_CAT_NAME = sprintf(_MD_D3XCGAL_INDEX_USERS_GAL, $row['uname']);
                        $row['parent'] = 1;
                        if (isset($result)) $xoopsDB->freeRecordSet($result);
                } else {
                    $result = $xoopsDB->query("SELECT name, parent FROM ".$xoopsDB->prefix($mydirname."_categories")." WHERE cid = '$cat'");
                        if ($xoopsDB->getRowsNum($result) == 0) redirect_header('index.php',2,_MD_D3XCGAL_NO_EXIST_CAT);
                        $row = $xoopsDB->fetchArray($result);
            $row['name']=$myts->makeTboxData4Show($row['name']);
                ...
                while($row['parent'] != 0){
                    $result = $xoopsDB->query("SELECT cid, name, parent FROM ".$xoopsDB->prefix($mydirname."_categories")." WHERE cid = '{$row['parent']}'");
                        if ($xoopsDB->getRowsNum($result) == 0) redirect_header('index.php',2,_MD_D3XCGAL_ORPHAN_CAT);
                        $row = $xoopsDB->fetchArray($result);
            $row['name']=$myts->makeTboxData4Show($row['name']);
...
function display_thumbnails($album, $cat, $spage, $thumbcols, $thumbrows, $display_tabs)
{
        global $xoopsModuleConfig, $xoopsTpl;
        $myts =& MyTextSanitizer::getInstance(); // MyTextSanitizer object

        $thumb_per_page = $thumbcols * $thumbrows;
        $lower_limit = ($spage-1) * $thumb_per_page;
        ...
        if (count($pic_data) > 0) {
                foreach ($pic_data as $key => $row) {
                        ...
                        $pic_title =_MD_D3XCGAL_FUNC_FNAME.$myts->makeTBoxData4Show($row['filename'])."\n".
                                _MD_D3XCGAL_FUNC_FSIZE.($row['filesize'] >> 10)._MD_D3XCGAL_KB."\n".
                                _MD_D3XCGAL_FUNC_DIM.$row['pwidth']."x".$row['pheight']."\n".
                                _MD_D3XCGAL_FUNC_DATE.formatTimestamp($row['ctime'],'m');
                ...
                theme_display_thumbnails($thumb_list, $thumb_count, $album_name, $album, $cat, $spage, $total_pages, is_numeric($album), $display_tabs);
        } else {
            $xoopsTpl->assign('no_img',1);
            $xoopsTpl->assign('lang_no_img',_MD_D3XCGAL_NO_IMG_TO_DISPLAY);
...
function get_pic_url(&$pic_row, $mode)
{
        ...
        if ((strtolower($ext) == ".jpg" or strtolower($ext) == ".jpeg") and $xoopsModuleConfig['watermarking']) {
                return XOOPS_URL.'/modules/'.$xoopsModule->getVar('dirname').'/index.php?page=watermark&picturename='.$url_prefix[$pic_row['url_prefix']]. path2url($pic_row['filepath']. $pic_prefix[$mode]. $pic_row['filename']);
        }
        else {
...
function get_full_real_path()
{
        ...
        if (empty($xoopsModuleConfig['realpath']))
                return XOOPS_ROOT_PATH.'/modules/'.$mydirname.'/'.$xoopsModuleConfig['fullpath'];
        else
                return $xoopsModuleConfig['realpath'];
}
