<?php
/**
 * @version $Id: entry.class.php 472 2008-06-13 07:23:27Z hodaka $
...
EOD;
    ...
    function pingExcerpt($length=255, $strip_tags=true) {
        ...
        if($strip_tags) {
            $excerpt = strip_tags($myts->displayTarea(strip_tags($this->getVar('excerpt', 'n')), $this->dohtml(), 1, $this->doxcode(), $this->doimage(), $this->dobr()));
        } else {
...
//                    $entry_handler->insert($this);
        ...
        // Anyway, re-calculate trackback count
       	return $this->synchronizeTrackbacks();
    }
...
*/
    ...
    function getDateSelbox() {
        ...
        if (file_exists(XOOPS_ROOT_PATH."/language/$language/calendar.php")) {
            require_once XOOPS_ROOT_PATH."/language/$language/calendar.php";
        } else {
