<?php
/**
 * @version $Id: xoops_version.php 467 2008-06-09 03:48:09Z hodaka $
...
$modversion['config'][$k]['formtype'] = 'select';
$modversion['config'][$k]['valuetype'] = 'text';
$modversion['config'][$k]['default'] = $mydirname;
$options = array();
$module_handler = & xoops_gethandler('module');
$criteria = new criteriaCompo('isactive', 1);
if(defined('XOOPS_CUBE_LEGACY'))
    $criteria->add(new criteria('mid', 5, '>'));
else
    $criteria->add(new criteria('mid', 1, '>'));
$modules =& $module_handler->getList($criteria, true);
foreach($modules as $dirname => $modulename) {
    if($dirname == $mydirname) {
        $options[$constpref.'_NO_COM_AGENT'] = $dirname;
    } else {
        $mytrustdirname_file = sprintf('%s/modules/%s/mytrustdirname.php', XOOPS_ROOT_PATH, $dirname);
        if(file_exists($mytrustdirname_file)) { // Are they d3 modules?
        	$options[$modulename] = $dirname;   // Not check if they are d3forum
        }    
    }
}
$modversion['config'][$k]['options'] = $options;
$k++;
