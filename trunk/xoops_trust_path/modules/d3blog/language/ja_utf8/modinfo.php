<?php
/**
 * @version $Id: modinfo.php 474 2008-06-13 10:55:53Z hodaka $
...
define ( $constpref.'_WYSIWYG_DSC','FCKeditorはhtml権限のあるユーザーのみ使用可' );
...
define ( $constpref.'_PERMED_DSC','エントリ単位の閲覧権限を設定した場合のグループ初期値です。<br /><span style="font-weight:bold;color:#009;">グループを追加した場合は、先にモジュールアップデートする必要があります。</span>' );
...
define ( $constpref.'_FIGUREHANDLER_DSC','d3blogでは画像・図表などをキャプションとともにマークアップするxcodeの[fig]タグ変換が組み込まれています。<br />さらに、jsを使うと画像サイズに応じてクラスを自動生成することが可能になります。<br />ただし、このjsは実験的な取り組みなので、テーマ操作やスタイルシートの知識がない場合はoffにしておいてください。' );
define ( $constpref.'_COM_AGENT','コメントシステムの選択' );
define ( $constpref.'_COM_AGENT_DSC','d3forumのコメント統合機能を使用する場合はフォーラムのモジュール名を指定します。' );
...
define ( $constpref.'_NOTIFY_GLOBAL_COMPOSTED','承認が必要なコメント投稿があったとき' );
define ( $constpref.'_NOTIFY_GLOBAL_COMPOSTED_CAP','ブログに承認が必要なコメント投稿があったら通知する' );
define ( $constpref.'_NOTIFY_GLOBAL_COMPOSTED_DSC','ブログに承認が必要なコメント投稿があったとき' );
define ( $constpref.'_NOTIFY_GLOBAL_COMPOSTED_SBJ','[{X_SITENAME}] {X_MODULE}: 承認が必要なコメントが投稿されました' );
define ( $constpref.'_NOTIFY_GLOBAL_COMAPPROVED','ブログに新規コメント掲載があったとき' );
define ( $constpref.'_NOTIFY_GLOBAL_COMAPPROVED_DSC','ブログに新規コメント掲載があったら通知する' );
define ( $constpref.'_NOTIFY_GLOBAL_COMAPPROVED_CAP','ブログに新規コメント掲載があったら通知する' );
define ( $constpref.'_NOTIFY_GLOBAL_COMAPPROVED_SBJ','[{X_SITENAME}] {X_MODULE}: ブログに新規コメントが掲載されました' );
define ( $constpref.'_NOTIFY_ENTRY_COMMENT','新規コメント投稿' );
define ( $constpref.'_NOTIFY_ENTRY_COMMENT_CAP','このエントリにコメントがついたら通知する' );
define ( $constpref.'_NOTIFY_ENTRY_COMMENT_DSC','このエントリにコメントがついたとき' );
define ( $constpref.'_NOTIFY_ENTRY_COMMENT_SBJ','[{X_SITENAME}] {X_MODULE}: コメントの投稿がありました' );
