<?php
/* 
 * class MyHtmlFilter based htmLawed 1.1
 */
...
// ===== begin htmLawed ========================
		...
		function hl_tag($t){
			...
			static $aNP = array(
				...
				'pluginurl'=>1,
				'usemap'=>1
			); // Attrs needing protocol check; for attrs like onmouseover & src, using: '$n[0] != 'o' && strpos($n, 'src') === false'; 'style' separately handled
			static $aNU = array(
				'class'=>array(
					'param'=>1,
					'script'=>1
			...
			foreach($aA as $k=>$v){
				if(!isset($C['deny_attribute'][$k]) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e])))){
					...
					// ===== end photosite =======================
						$v = preg_replace_callback('`((?:u|&#(?:x75|117);)(?:r|&#(?:x72|114);)(?:l|&#(?:x6c|108);)(?:\(|&#(?:x28|40);)(?: |&#(?:x20|32);)*(?:\'|"|&(?:quot|apos|34|39|x22|x27);)?)(.+)((?:\'|"|&(?:quot|apos|34|39|x22|x27);)?(?: |&#(?:x20|32);)*(?:\)|&#(?:x29|41);))`iS', array( &$this, 'hl_prot'), $v);
						if(!$C['css_expression']){
