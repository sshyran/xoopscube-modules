...
if (! function_exists('b_d3downloads_list_show') ) {
	function b_d3downloads_list_show( $options )
	{
		...
		$selected_order = empty( $options[2] ) || ! in_array( $options[2] , b_d3downloads_list_order() ) ? 'd.date DESC' : $options[2] ;
		...
		$downdata = $block_download->get_downdata_for_block( $whr, $max_entry, 0, $date_format, 0, $selected_order, $whr_categories );
		if( ! empty( $downdata ) ){
...
if (! function_exists('b_d3downloads_list_order') ) {
	function b_d3downloads_list_order()
	{
		return array(
			'd.hits' ,
			'd.hits DESC' ,
