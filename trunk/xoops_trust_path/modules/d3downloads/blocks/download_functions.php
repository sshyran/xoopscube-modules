...
if (! function_exists('b_d3downloads_download_edit') ) {
	function b_d3downloads_download_edit( $options )
	{
		...
		// get downloads_title
		$downloaddata = array( 0 => '--' ) ;
		$result = $db->query( "SELECT lid, title FROM ".$db->prefix( $mydirname."_downloads" )." ORDER BY lid" ) ;
		while( list( $id , $title ) = $db->fetchRow( $result ) ) {
			$downloaddata[ $id ] = sprintf('%06d',$id).': '.$myts->makeTboxData4Show( $title ) ;
		}
