...
//from DB
$sql = "SELECT i.lid, i.cid, i.uid, i.description, i.regdate, i.clicks, i.ASIN, i.title, i.Creator, i.Manufacturer, i.ProductGroup, i.DetailPageURL, i.MediumImage, i.IsAdult, c.ctitle FROM $table_items i LEFT JOIN $table_cat c ON i.cid=c.cid WHERE i.stats>0 AND i.cid=$cid ORDER BY i.regdate DESC";
...
if( $cid > 0 ) $xoopsTpl->assign( 'subcategories' , ma_get_sub_categories( $cid , $cattree ) ) ;
$xoopsTpl->assign( 'category_options' , ma_get_cat_options('corder',$cid) ) ;
...
$xoopsTpl->assign( 'xoops_pagetitle' , $xoopsModule->getVar('name')." : ".$bread[count($bread)-1]['name'] );
