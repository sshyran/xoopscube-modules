...
if(! empty($_POST['action']) && $_POST['action'] == 'delete' && isset($_POST['ids']) && is_array($_POST['ids'])) {
	...
	redirect_header("index.php?page=photomanager&num=$num&cid=$cid" , 2 , _MD_D3IMGTAG_DELETINGPHOTO);
	exit;
	...
	redirect_header("index.php?page=photomanager&num=$num&cid=$cid" , 2 , _MD_D3IMGTAG_DBUPDATED);
	exit;
...
$prs = $xoopsDB->query("SELECT l.lid, l.img, l.title, l.submitter, l.ext, l.res_x, l.res_y, l.status FROM $table_photos l LEFT JOIN $table_cat c ON l.cid=c.cid WHERE $whr ORDER BY l.lid DESC LIMIT $pos,$num");
...
$photonavinfo = sprintf(_MD_D3IMGTAG_AM_PHOTONAVINFO , $pos + 1 , $last , $numrows);

// Options for the number of photos in a display
$numbers = explode('|' , $d3imgtag_perpage);
$num_options = '' ;
foreach($numbers as $number) {
	$number = intval($number);
	if($number < 1) continue ;
	$selected = $number == $num ? "selected='selected'" : "";
	$num_options .= "<option value='$number' $selected>".sprintf(_MD_D3IMGTAG_FMT_PHOTONUM,$number)."</option>\n";
}

d3imgtag_get_cat_options();

// Options for Selecting a category
$cat_options = d3imgtag_get_cat_options('title' , $cid , '--' , '----');
$cat_options_for_update = d3imgtag_get_cat_options('title' , 0 , '--' , _AM_D3IMGTAG_OPT_NOCHANGE);

// Options for Selecting a user
$user_options = "<option value='0'>"._AM_D3IMGTAG_OPT_NOCHANGE."</option>\n";
...
echo "<h3 style='text-align:left;'>".sprintf(_AM_D3IMGTAG_H3_FMT_PHOTOMANAGER,$xoopsModule->name())."</h3>\n";
...
<p align='center' style='margin:0px;'>
  $photonavinfo
  <a href='../index.php?page=submit&cid=$cid'><img src='../images/img_add.png' width='16' height='16' alt='"._AM_D3IMGTAG_CAT_LINK_ADDPHOTOS."' title='"._AM_D3IMGTAG_CAT_LINK_ADDPHOTOS."' /></a>
</p>
...
// list part
$col = 0 ;
while(list($lid , $img, $title , $submitter , $ext , $w , $h , $status) = $xoopsDB->fetchRow($prs)) {
	$title = $myts->makeTboxData4Show($title);

	if(in_array(strtolower($ext) , $d3imgtag_normal_exts)) {
		$imgsrc_thumb = "$thumbs_url/$img.$ext";
		$ahref_photo = "$photos_url/$img.$ext";
		$widthheight = $w > $h ? "width='$d3imgtag_thumbsize'" : "height='$d3imgtag_thumbsize'";
	} else {
		$imgsrc_thumb = "$thumbs_url/$img.gif";
		$ahref_photo = "$photos_url/$img.$ext";
		$widthheight = '' ;
	}
	...
	$deadlinkbutton = is_readable("$photos_dir/{$img}.{$ext}") ? "" : "<img src='".XOOPS_URL."/modules/$mydirname/images/dead.png' border='0' alt='"._MD_D3IMGTAG_AM_DEADLINKMAINPHOTO."' title='"._MD_D3IMGTAG_AM_DEADLINKMAINPHOTO."' />";
	...
	echo "
		<td align='center' style='background-color:$bgcolor; margin: 0 px; padding: 3 px; border-width:0px 2px 2px 0px; border-style: solid; border-color:black;'>
			<table border='0' cellpadding='0' cellmargin='0'>
				<tr>
					<td></td>
					<td><img src='../images/pixel_trans.gif' width='$d3imgtag_thumbsize' height='1' alt='' /></td>
					<td></td>
				</tr>
				<tr>
					<td><img src='../images/pixel_trans.gif' width='1' height='$d3imgtag_thumbsize' alt='' /></td>
					<td align='center'><a href='$ahref_photo' target='_blank'><img src='$imgsrc_thumb' border='0' alt='$title' title='$title' /></a></td>
					<td><img src='../images/pixel_trans.gif' width='1' height='$d3imgtag_thumbsize' alt='' /></td>
				</tr>
				<tr>
					<td></td>
...
<tr>
	<td align='left'>
		<input type='button' value='"._MD_D3IMGTAG_BTN_SELECTNONE."' onclick=\"with(document.MainForm){for(i=0;i<length;i++){if(elements[i].type=='checkbox'){elements[i].checked=false;}}}\" />
		&nbsp;
		<input type='button' value='"._MD_D3IMGTAG_BTN_SELECTALL."' onclick=\"with(document.MainForm){for(i=0;i<length;i++){if(elements[i].type=='checkbox'){elements[i].checked=true;}}}\" />
	</td>
	<td align='right'>
		<input type='hidden' name='action' value='' />
		"._MD_D3IMGTAG_AM_LABEL_REMOVE."<input type='button' value='"._MD_D3IMGTAG_AM_BUTTON_REMOVE."' onclick='if(confirm(\""._MD_D3IMGTAG_AM_JS_REMOVECONFIRM."\")){document.MainForm.action.value=\"delete\"; submit();}' />
	</td>
</tr>
</table>
<br />
<table class='outer' style='width:100%;'>
	<tr>
		<th colspan='2'>"._AM_D3IMGTAG_TH_BATCHUPDATE."</th>
	</tr>
	<tr>
		<td class='head'>"._AM_D3IMGTAG_TH_TITLE."</td>
		<td class='even'><input type='text' name='new_title' size='50' /></td>
	</tr>
	<tr valign='top'>
		<td class='head'>"._AM_D3IMGTAG_TH_DESCRIPTION."</td>
		<td class='even'><textarea name='new_desc_text' cols='50' rows='5'></textarea></td>
	</tr>
	<tr>
		<td class='head'>"._AM_D3IMGTAG_TH_CATEGORIES."</td>
		<td class='even'>
			<select name='new_cid'>
	...
	<tr>
		<td class='head'>"._AM_D3IMGTAG_TH_SUBMITTER."</td>
		<td class='even'>
	...
	<tr valign='top'>
		<td class='head'>"._AM_D3IMGTAG_TH_DATE."</td>
		<td class='even'><input type='text' name='new_post_date' size='20' value='".formatTimestamp(time(),_MD_D3IMGTAG_DTFMT_YMDHI)."'></textarea></td>
	</tr>
	<tr>
		<td class='head'></td>
		<td class='even'><input type='submit' name='update' value='"._MD_D3IMGTAG_AM_BUTTON_UPDATE."' onclick='return confirm(\""._AM_D3IMGTAG_JS_UPDATECONFIRM."\")' tabindex='1' /></td>
	</tr>
</table>
</form>
";
