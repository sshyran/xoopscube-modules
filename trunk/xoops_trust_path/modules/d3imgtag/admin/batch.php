...
// reject Not Admin
if(! $isadmin) {
	redirect_header($mod_url.'/' , 2 , _MD_D3IMGTAG_MUSTREGFIRST);
	exit;
}
...
if($count < 1) {
	redirect_header(XOOPS_URL."/modules/$mydirname/admin/index.php", 2, _MD_D3IMGTAG_MUSTADDCATFIRST);
	exit();
}
...
if(isset($_POST['submit']) && $_POST['submit'] != "") {
	...
	if( empty( $dir ) || ! is_dir( $dir ) ) {
		...
		if( ! is_dir( $dir ) ) {
			redirect_header( 'index.php?page=batch' , 3 , _MD_D3IMGTAG_MES_INVALIDDIRECTORY . "<br />$dir4edit" ) ;
			exit ;
		}
	}
	...
	foreach( $file_names as $file_name ) {
		...
		if( is_readable( $file_path ) && in_array( strtolower( $ext ) , $array_allowed_exts ) ) {
			...
			$fileName = createFileName($d3imgtag_minfile, $d3imgtag_maxfile, $d3imgtag_filerule);	//@d3imgtag
			$fileSize = filesize($file_path);	//@d3imgtag
			$sql = "INSERT INTO $table_photos SET lid='$lid', img='$fileName', cid='$cid', title='$title', ext='$ext', share='$shareimg', size='$fileSize', res_x='$w', res_y='$h', submitter='$submitter', status=1, date='$date'" ;
			$xoopsDB->query( $sql ) or die( "DB error: INSERT photos table." ) ;
			if( $lid == 0 ) {
				$lid = $xoopsDB->getInsertId() ;
			}
			print " &nbsp; <a href='../index.php?page=photo&lid=$lid' target='_blank'>$file_path</a>\n" ;
			copy( $file_path , "$photos_dir/$lid.$ext" ) ;
			d3imgtag_modify_photo("$photos_dir/$lid.$ext" , "$photos_dir/$fileName.$ext") ;
			//d3imgtag_create_thumb( "$photos_dir/$lid.$ext" , $fileName , $ext ) ;
			d3imgtag_create_thumb( "$photos_dir/$fileName.$ext" , $fileName , $ext ) ;
			d3imgtag_create_preview("$photos_dir/$fileName.$ext" , $fileName , $ext);
			$xoopsDB->query( "INSERT INTO $table_text SET lid='$lid', description='$desc4save'" ) ;
			echo _AM_D3IMGTAG_MB_FINISHED."<br />\n" ;
	...
	if( $filecount <= 1 ) {
		echo "<p>$dir4edit : "._MD_D3IMGTAG_MES_BATCHNONE."</p>" ;
	} else {
		printf( "<p>"._MD_D3IMGTAG_MES_BATCHDONE."</p>" , $filecount - 1 ) ;
	}
...
// Make form objects
$form = new XoopsThemeForm(_MD_D3IMGTAG_PHOTOBATCHUPLOAD , "batchupload" , "index.php?page=batch");

$title_text = new XoopsFormText("" , "title" , 50 , 255 , $title4edit);
$title_tray = new XoopsFormElementTray(_AM_D3IMGTAG_TH_TITLE , '<br /><br />');
$title_tray->addElement($title_text);
$title_tray->addElement(new XoopsFormLabel("" , _MD_D3IMGTAG_BATCHBLANK));

$cat = new XoopsFormSelect(_AM_D3IMGTAG_TH_CATEGORIES , "cid" , $cid);
...
$submitter_select = new XoopsFormSelectUser(_AM_D3IMGTAG_TH_SUBMITTER, 'submitter', false, $submitter);

$date_text = new XoopsFormText(_AM_D3IMGTAG_TH_DATE, 'post_date', 20, 20, formatTimestamp(time(), _MD_D3IMGTAG_DTFMT_YMDHI));

$dir_tray = new XoopsFormElementTray(_MD_D3IMGTAG_TEXT_DIRECTORY , '<br /><br />');
$dir_text = new XoopsFormText(_MD_D3IMGTAG_PHOTOPATH , "dir", 50, 255 , $dir4edit);
$dir_tray->addElement($dir_text);
$dir_tray->addElement(new XoopsFormLabel(_MD_D3IMGTAG_DESC_PHOTOPATH));
$desc_tarea = new XoopsFormDhtmlTextarea(_AM_D3IMGTAG_TH_DESCRIPTION , 'desc' , $desc4edit , 10 , 50);
...
echo "<h3 style='text-align:left;'>".sprintf(_AM_D3IMGTAG_H3_FMT_BATCHREGISTER, $xoopsModule->name())."</h3>\n";
echo "<h5 style='text-align:left;color:#FF0000;'>"._AM_D3IMGTAG_H3_FMT_BATCHREGISTERNOTICE."</h5>\n";
d3imgtag_opentable();
