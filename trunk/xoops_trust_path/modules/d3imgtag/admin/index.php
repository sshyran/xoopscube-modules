...
function display_edit_form($cat_array , $form_title , $action)
{
	...
	// Title
	$form->addElement(new XoopsFormText(_AM_D3IMGTAG_CAT_TH_TITLE , 'title' , 30 , 50 , $myts->htmlSpecialChars($title)) , true);

	// Image URL
	$form->addElement(new XoopsFormText(_AM_D3IMGTAG_CAT_TH_IMGURL , 'imgurl' , 50 , 150 , $myts->htmlSpecialChars($imgurl)));
	...
	ob_end_clean();
	$form->addElement(new XoopsFormLabel(_AM_D3IMGTAG_CAT_TH_PARENT , $cat_selbox));
...
if($action == "insert") {
	...
	redirect_header("index.php" , 1 , _AM_D3IMGTAG_CAT_INSERTED);
	exit;
	...
	redirect_header("index.php" , 1 , _AM_D3IMGTAG_CAT_UPDATED);
	exit;
	...
	foreach($children as $child) {
		$whr .= "$child,";
		xoops_notification_deletebyitem($d3imgtag_mid , 'category' , $child);
	}
	$whr .= "$cid)";
	xoops_notification_deletebyitem($d3imgtag_mid , 'category' , $cid);

	d3imgtag_delete_photos($whr);

	$xoopsDB->query("DELETE FROM $table_cat WHERE $whr") or die("DB error: DELETE cat table");
	redirect_header('index.php' , 2 , _MD_D3IMGTAG_CATDELETED);
	exit;
...
echo "<h3 style='text-align:left;'>".sprintf(_AM_D3IMGTAG_H3_FMT_CATEGORIES , $xoopsModule->name())."</h3>\n";
...
if($disp == "edit" && $cid > 0) {
	...
	display_edit_form($cat_array , _AM_D3IMGTAG_CAT_MENU_EDIT , 'update');

} else if($disp == "new") {

	// New
	$cat_array = array('cid' => 0 , 'pid' => $cid , 'title' => '' , 'imgurl' => 'http://');
	display_edit_form($cat_array , _AM_D3IMGTAG_CAT_MENU_NEW , 'insert');
	...
	$link_admission = $waiting > 0 ? sprintf(_AM_D3IMGTAG_CAT_FMT_NEEDADMISSION , $waiting) : '' ;

	// Top links
	echo "<p><a href='?disp=new&cid=0'>"._AM_D3IMGTAG_CAT_LINK_MAKETOPCAT."<img src='../images/cat_add.png' width='16' height='16' alt='"._AM_D3IMGTAG_CAT_LINK_MAKETOPCAT."' title='"._AM_D3IMGTAG_CAT_LINK_MAKETOPCAT."' /></a> &nbsp;  &nbsp; <a href='index.php?page=admission' style='color:red;'>$link_admission</a></p>\n";
	...
	<table width='75%' class='outer' cellpadding='4' cellspacing='1'>
	  <tr valign='middle'>
	    <th>"._AM_D3IMGTAG_CAT_TH_TITLE."</th>
	    <th>"._AM_D3IMGTAG_CAT_TH_PHOTOS."</th>
	    <th>"._AM_D3IMGTAG_CAT_TH_OPERATION."</th>
	    <th nowrap='nowrap'>"._AM_D3IMGTAG_CAT_TH_IMAGE."</th>
	  </tr>
	";
	...
	foreach($cat_tree_array as $cat_node) {
		...
		$cid = intval($cid);
		$del_confirm = 'confirm("' . sprintf(_AM_D3IMGTAG_CAT_FMT_CATDELCONFIRM , $title) . '")' ;
		...
		echo "
	  <tr>
	    <td class='$oddeven' width='100%'><a href='index.php?page=photomanager&cid=$cid'>$prefix&nbsp;".$myts->htmlSpecialChars($title)."</a></td>
	    <td class='$oddeven' nowrap='nowrap' align='right'>
	      <a href='index.php?page=photomanager&cid=$cid'>$photos_num</a>
	      <a href='../index.php?page=submit&cid=$cid'><img src='../images/img_add.png' width='16' height='16' alt='"._AM_D3IMGTAG_CAT_LINK_ADDPHOTOS."' title='"._AM_D3IMGTAG_CAT_LINK_ADDPHOTOS."' /></a></td>
	    <td class='$oddeven' align='center' nowrap='nowrap'>
	      &nbsp;
	      <a href='?disp=edit&amp;cid=$cid'><img src='../images/cat_edit.png' width='16' height='16' alt='"._AM_D3IMGTAG_CAT_LINK_EDIT."' title='"._AM_D3IMGTAG_CAT_LINK_EDIT."' /></a>
	      &nbsp;
	      <a href='?disp=new&amp;cid=$cid'><img src='../images/cat_add.png' width='16' height='16' alt='"._AM_D3IMGTAG_CAT_LINK_MAKESUBCAT."' title='"._AM_D3IMGTAG_CAT_LINK_MAKESUBCAT."' /></a>
	      &nbsp;
	...
	echo "
	  <!-- <tr>
	    <td colspan='4' align='right' class='foot'><input type='submit' name='batch_update' value='"._AM_D3IMGTAG_CAT_BTN_BATCH."' /></td>
	  </tr> -->
	</table>
	</form>
	";
