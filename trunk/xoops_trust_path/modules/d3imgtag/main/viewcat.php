...
$num = empty($_GET['num']) ? intval($d3imgtag_perpage) : intval($_GET['num']);
if($num < 1) $num = 10 ;
$pos = empty($_GET['pos']) ? 0 : intval($_GET['pos']);
$view = empty($_GET['view']) ? $d3imgtag_viewcattype : $_GET['view'] ;
...
if($view == 'table') {
	...
	$function_assigning = 'd3imgtag_get_array_for_photo_assign_light' ;
} else {
	$xoopsOption['template_main'] = "{$mydirname}_viewcat_list.html";
	$function_assigning = 'd3imgtag_get_array_for_photo_assign' ;
}
...
if($cid > 0) {
	...
	$xoopsTpl->assign('subcategories' , d3imgtag_get_sub_categories($cid , $cattree));	$xoopsTpl->assign('category_options' , d3imgtag_get_cat_options());

	$cids = $cattree->getAllChildId($cid);
	array_push($cids , $cid);
	$photo_total_sum = d3imgtag_get_photo_total_sum_from_cats($cids , "status>0");
	$sub_title = preg_replace("/\'\>/" , "'><img src='$mod_url/images/folder.png' alt='' align='absmiddle'/>" , $cattree->getNicePathFromId($cid , 'title' , "index.php?page=viewcat&num=$num"));
	...
	if($uid < 0) {
		...
		$xoopsTpl->assign('album_sub_title' , _MD_D3IMGTAG_TEXT_SMNAME4);
		$xoopsTpl->assign( 'xoops_pagetitle' , _MD_D3IMGTAG_TEXT_SMNAME4 ) ;
	// uid Specified
	} else {
		$where = "submitter=$uid";
		$get_append = "uid=$uid";
		$xoopsTpl->assign('uid' , $uid);
		$xoopsTpl->assign('album_sub_title' , "<img src='$mod_url/images/img_more.png' alt='' />" . d3imgtag_get_name_from_uid($uid));
		$xoopsTpl->assign( 'xoops_pagetitle' , d3imgtag_get_name_from_uid( $uid ) ) ;
	}
...
if($photo_small_sum > 0) {
	...
	$prs = $xoopsDB->query("SELECT l.lid, l.img, l.size, l.cid, l.title, l.ext, l.share, l.res_x, l.res_y, l.status, l.date, l.hits, l.rating, l.votes, l.comments, l.submitter, t.description $select_append FROM $table_photos l USE INDEX (date) INNER JOIN $table_text t ON l.lid=t.lid $join_append WHERE $where AND l.status>0 ORDER BY {$d3imgtag_orders[$orderby][0]}" , $num , $pos);
	if( ! $prs ) {
		$prs = $xoopsDB->query( "SELECT l.lid, l.img, l.size, l.cid, l.title, l.ext, l.share, l.res_x, l.res_y, l.status, l.date, l.hits, l.rating, l.votes, l.comments, l.submitter, t.description $select_append FROM $table_photos l INNER JOIN $table_text t ON l.lid=t.lid $join_append WHERE $where AND l.status>0 ORDER BY {$d3imgtag_orders[$orderby][0]}" , $num , $pos ) ;
	}
	...
	if($photo_small_sum > 1) {
		...
		$xoopsTpl->assign('lang_sortby' , _MD_D3IMGTAG_SORTBY);
		$xoopsTpl->assign('lang_title' , _MD_D3IMGTAG_TITLE);
		$xoopsTpl->assign('lang_date' , _MD_D3IMGTAG_DATE);
		$xoopsTpl->assign('lang_rating' , _MD_D3IMGTAG_RATING);
		$xoopsTpl->assign('lang_popularity' , _MD_D3IMGTAG_POPULARITY);
		$xoopsTpl->assign('lang_cursortedby' , sprintf(_MD_D3IMGTAG_CURSORTEDBY , $d3imgtag_orders[$orderby][1]));

		$nav = new XoopsPageNav($photo_small_sum , $num , $pos , 'pos' , "page=viewcat&$get_append&num=$num&orderby=$orderby");
		$nav_html = $nav->renderNav(10);
		$last = $pos + $num ;
		if($last > $photo_small_sum) $last = $photo_small_sum ;
		$photonavinfo = sprintf(_MD_D3IMGTAG_AM_PHOTONAVINFO , $pos + 1 , $last , $photo_small_sum);
