...
if(! ($global_perms & D3IMGTAG_GPERM_RATEVOTE))
{
...
if(! empty($_POST['submit'])) {
	...
	if($rating <= 0 || $rating > 10) {
		redirect_header("index.php?page=ratephoto&lid=$lid" , 4 , _MD_D3IMGTAG_NORATING);
		exit;
	}
	...
	if($ratinguser != 0) {
		...
		if($is_my_photo) {
			redirect_header("index.php" , 4 , _MD_D3IMGTAG_CANTVOTEOWN);
			exit;
		}
		...
		if($has_already_rated) {
			redirect_header("index.php" , 4 , _MD_D3IMGTAG_VOTEONCE2);
			exit;
		}
	...
	d3imgtag_updaterating($lid);
	$ratemessage = _MD_D3IMGTAG_VOTEAPPRE."<br />".sprintf(_MD_D3IMGTAG_THANKURATE , $xoopsConfig['sitename']);
	...
	$rs = $xoopsDB->query("SELECT l.lid, l.img, l.cid, l.title, l.ext, l.share, l.size, l.res_x, l.res_y, l.status, l.date, l.hits, l.rating, l.votes, l.comments, l.submitter, t.description FROM $table_photos l LEFT JOIN $table_text t ON l.lid=t.lid  WHERE l.lid='$lid' AND l.status>0");
	if($rs == false || $xoopsDB->getRowsNum($rs) <= 0) {
		redirect_header("index.php" , 2 , _MD_D3IMGTAG_NOMATCH);
		exit;
	}

	$fetched_result_array = $xoopsDB->fetchArray($rs);
	$xoopsTpl->assign('photo' , d3imgtag_get_array_for_photo_assign($fetched_result_array));

	include(dirname(dirname(__FILE__)).'/include/assign_globals.php');
	$xoopsTpl->assign($d3imgtag_assign_globals);

	$xoopsTpl->assign('lang_voteonce' , _MD_D3IMGTAG_VOTEONCE);
	$xoopsTpl->assign('lang_ratingscale' , _MD_D3IMGTAG_RATINGSCALE);
	$xoopsTpl->assign('lang_beobjective' , _MD_D3IMGTAG_BEOBJECTIVE);
	$xoopsTpl->assign('lang_donotvote' , _MD_D3IMGTAG_DONOTVOTE);
	$xoopsTpl->assign('lang_rateit' , _MD_D3IMGTAG_RATEIT);
