...
if(! defined('D3IMGTAG_BLOCK_TOPNEWS_INCLUDED')) {

define('D3IMGTAG_BLOCK_TOPNEWS_INCLUDED' , 1);

function b_d3imgtag_topnews_show($options)
{
	global $xoopsDB ;
	...
	$this_template = 'db:'.$mydirname.'_block_topnews.html';
	...
	$result = $xoopsDB->query("SELECT l.lid , l.img, l.cid , l.title , l.ext , l.res_x , l.res_y , l.submitter , l.status , l.date AS unixtime , l.hits , l.rating , l.votes , l.comments , c.title AS cat_title FROM $table_photos l LEFT JOIN $table_cat c ON l.cid=c.cid WHERE l.status>0 AND $whr_cat ORDER BY unixtime DESC" , $photos_num , 0);
	$count = 1 ;
...
function b_d3imgtag_topnews_edit($options)
{
	global $xoopsDB ;
...
function b_d3imgtag_topnews_p_show($options)
{
	global $xoopsDB ;
	...
	$this_template = 'db:'.$mydirname.'_block_topnews_p.html';
...
function b_d3imgtag_topnews_p_edit($options)
{
	global $xoopsDB ;
