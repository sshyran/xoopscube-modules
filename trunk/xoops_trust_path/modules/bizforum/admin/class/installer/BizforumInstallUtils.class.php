...
if(class_exists('Bizforum_InstallUtils'))
{
    return;
}

/**
 * Bizforum_InstallUtils
**/
class Bizforum_InstallUtils
{
    /**
    ...
    **/
    public static function installSQLAutomatically(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(!$scanner->loadFile($sqlFilePath))
        {
            $log->addError(
                XCube_Utils::formatString(
                    _MI_BIZFORUM_INSTALL_ERROR_SQL_FILE_NOT_FOUND,
                    $sqlFile
                )
            );
            return false;
        ...
        $log->addReport(_MI_BIZFORUM_INSTALL_MSG_DB_SETUP_FINISHED);
        return true;
    }
    ...
    public static function DBquery(/*** string ***/ $query,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($sqls as $sql)
        {
            if($root->mController->mDB->query($sql))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_MSG_SQL_SUCCESS,
                        $sql
                    )
                );
            }
            else
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_MSG_SQL_ERROR,
                        $sql
                    )
                );
                $successFlag = false;
    ...
    **/
    public static function installAllOfModuleTemplates(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(is_array($templates) && count($templates) > 0)
        {
            ...
            {
                Bizforum_InstallUtils::installModuleTemplate($module,$template,$log);
            }
        }
    }
    ...
    **/
    public static function installModuleTemplate(/*** XoopsModule ***/ &$module,/*** string[] ***/ $template,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        $tplHandler =& Bizforum_Utils::getXoopsHandler('tplfile');
        $filename   =  Bizforum_InstallUtils::replaceDirname(trim($template['file']),$dirname,$trustDirname);
        $tplData    =  Bizforum_InstallUtils::readTemplateFile($dirname,$trustDirname,$filename['trust']);
        ...
        if($tplHandler->insert($tplFile))
        {
            $log->addReport(
                XCube_Utils::formatString(
                    _MI_BIZFORUM_INSTALL_MSG_TPL_INSTALLED,
                    $filename['public']
                )
            );
        }
        else
        {
            $log->addError(
                XCube_Utils::formatString(
                    _MI_BIZFORUM_INSTALL_ERROR_TPL_INSTALLED,
                    $filename['public']
                )
            );
            return false;
        ...
        if(is_array($delTemplates) && count($delTemplates) > 0)
        {
            ...
            foreach($delTemplates as $tpl)
            {
                if(!$tplHandler->delete($tpl))
                {
                    $log->addError(
                        XCube_Utils::formatString(
                            _MI_BIZFORUM_INSTALL_ERROR_TPL_UNINSTALLED,
                            $tpl->get('tpl_file')
                        )
                    );
                }
    ...
    **/
    public static function installAllOfBlocks(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(is_array($blocks) && count($blocks) > 0)
        {
            ...
            {
                $newBlock =& Bizforum_InstallUtils::createBlockByInfo($module,$block);
                Bizforum_InstallUtils::installBlock($module,$newBlock,$block,$log);
            }
        }
        return true;
    }
    ...
    **/
    public static function &createBlockByInfo(/*** XoopsModule ***/ &$module,/*** string[] ***/ $block)
    {
        ...
        $filename = isset($block['template']) ?
            Bizforum_InstallUtils::replaceDirname($block['template'],$module->get('dirname')) :
            null;
    
        $blockHandler =& Bizforum_Utils::getXoopsHandler('block');
    ...
    **/
    public static function installBlock(/*** XoopsModule ***/ &$module,/*** XoopsBlock ***/ &$blockObj,/*** string[] ***/ &$block,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(!$blockHandler->insert($blockObj,$autoLink))
        {
            $log->addError(
                XCube_Utils::formatString(
                    _MI_BIZFORUM_INSTALL_ERROR_BLOCK_INSTALLED,
        ...
        $log->addReport(
            XCube_Utils::formatString(
                _MI_BIZFORUM_INSTALL_MSG_BLOCK_INSTALLED,
                $blockObj->getVar('name')
            )
        );
    
        Bizforum_InstallUtils::installBlockTemplate($blockObj,$module,$log);
        ...
        if($autoLink)
        {
            ...
            if(!$blockHandler->db->query($sql))
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_ERROR_BLOCK_COULD_NOT_LINK,
                        $blockObj->getVar('name')
                    )
                );
            }
        ...
        $gpermHandler =& Bizforum_Utils::getXoopsHandler('groupperm');
        ...
        {
            $memberHandler =& Bizforum_Utils::getXoopsHandler('member');
            ...
            foreach($groups as $group)
            {
                ...
                if(!$gpermHandler->insert($perm))
                {
                    $log->addWarning(
                        XCube_Utils::formatString(
                            _MI_BIZFORUM_INSTALL_ERROR_PERM_COULD_NOT_SET,
                            $blockObj->getVar('name')
                        )
                    );
                }
        ...
        else
        {
            ...
            foreach($groups as $group)
            {
                ...
                if(!$gpermHandler->insert($perm))
                {
                    $log->addWarning(
                        XCube_Utils::formatString(
                            _MI_BIZFORUM_INSTALL_ERROR_BLOCK_PERM_SET,
                            $blockObj->getVar('name')
                        )
                    );
                }
    ...
    **/
    public static function installBlockTemplate(/*** XoopsBlock ***/ &$block,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        $filename = Bizforum_InstallUtils::replaceDirname(
        ...
        $tplSource = Bizforum_InstallUtils::readTemplateFile(
            $module->get('dirname'),
            $module->getInfo('trust_dirname'),
            $filename['trust'],
            true
        ...
        if($tplHandler->insert($tplFile))
        {
            $log->addReport(
                XCube_Utils::formatString(
                    _MI_BIZFORUM_INSTALL_MSG_BLOCK_TPL_INSTALLED,
                    $filename['public']
                )
            );
            return true;
        ...
        $log->addError(
            XCube_Utils::formatString(
                _MI_BIZFORUM_INSTALL_ERROR_BLOCK_TPL_INSTALLED,
                $filename['public']
            )
        );
        return false;
    ...
    public static function uninstallAllOfBlocks(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($blocks as $block)
        {
            if($blockHandler->delete($block))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_MSG_BLOCK_UNINSTALLED,
                        $block->get('name')
                    )
                );
            }
            else
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_ERROR_BLOCK_UNINSTALLED,
                        $block->get('name')
                    )
                );
                $successFlag = false;
            ...
            if(!$gpermHandler->deleteAll($cri))
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_ERROR_BLOCK_PERM_DELETE,
                        $block->get('name')
                    )
                );
                $successFlag = false;
        ...
        foreach($blocks->mBlocks as $block)
        {
            switch($block->mStatus)
            {
                case LEGACY_INSTALLINFO_STATUS_LOADED:
                    Bizforum_InstallUtils::updateBlockTemplateByInfo($block,$module,$log);
                    break;
                case LEGACY_INSTALLINFO_STATUS_UPDATED:
                    Bizforum_InstallUtils::updateBlockByInfo($block,$module,$log);
                    break;
                case LEGACY_INSTALLINFO_STATUS_NEW:
                    Bizforum_InstallUtils::installBlockByInfo($block,$module,$log);
                    break;
                case LEGACY_INSTALLINFO_STATUS_DELETED:
                    Bizforum_InstallUtils::uninstallBlockByFuncNum($block->mFuncNum,$module,$log);
                    break;
                default:
                    break;
            }
    ...
    **/
    ...
    {
        $blockHandler =& Bizforum_Utils::getModuleHandler('newblocks','legacy');
        $cri =& new CriteriaCompo();
        ...
        foreach($blocks as $block)
        {
            Bizforum_InstallUtils::uninstallBlockTemplate($block,$module,$log,true);
            Bizforum_InstallUtils::installBlockTemplate($block,$module,$log);
        }
    }
    ...
    public static function updateBlockByInfo(/*** Legacy_BlockInformation ***/ &$info,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($blocks as $block)
        {
            ...
            if($blockHandler->insert($block))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_MSG_BLOCK_UPDATED,
                        $block->get('name')
                    )
                );
            }
            else
            {
                $log->addError(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_ERROR_BLOCK_UPDATED,
                        $block->get('name')
                    )
                );
            }
    ...
    **/
    public static function uninstallBlockTemplate(/*** XoopsBlock ***/ &$block,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log,/*** bool ***/ $defaultOnly = false)
    {
        ...
        $log->addReport(
            XCube_Utils::formatString(
                _MI_BIZFORUM_INSTALL_MSG_BLOCK_TPL_UNINSTALLED,
                $block->get('template')
            )
        );
        return true;
    ...
    public static function installAllOfConfigs(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($preferences->mPreferences as $info)
        {
            ...
            if($configHandler->insertConfig($config))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_MSG_CONFIG_ADDED,
            ...
            else
            {
                $log->addError(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_ERROR_CONFIG_ADDED,
                        $config->get('conf_name')
                    )
                );
                $successFlag = false;
    ...
    public static function uninstallAllOfConfigs(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($configs as $config)
        {
            if($configHandler->deleteConfig($config))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_MSG_CONFIG_DELETED,
            ...
            else
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_BIZFORUM_INSTALL_ERROR_CONFIG_DELETED,
                        $config->getVar('conf_name')
                    )
                );
                $sucessFlag = false;
    ...
    **/
    public static function smartUpdateAllOfConfigs(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($configs->mPreferences as $config)
        {
            ...
            {
                case LEGACY_INSTALLINFO_STATUS_UPDATED:
                    Bizforum_InstallUtils::updateConfigByInfo($config,$module,$log);
                    break;
                case LEGACY_INSTALLINFO_STATUS_ORDER_UPDATED:
                    Bizforum_InstallUtils::updateConfigOrderByInfo($config,$module,$log);
                    break;
                case LEGACY_INSTALLINFO_STATUS_NEW:
                    Bizforum_InstallUtils::installConfigByInfo($config,$module,$log);
                    break;
                case LEGACY_INSTALLINFO_STATUS_DELETED:
                    Bizforum_InstallUtils::uninstallConfigByOrder($config->mOrder,$module,$log);
                    break;
                default:
                    break;
            }
    ...
    **/
    public static function updateConfigByInfo(/*** Legacy_PreferenceInformation ***/ &$info,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        {
            $log->addError(_MIBIZFORUM_INSTALL_ERROR_CONFIG_NOT_FOUND);
            return false;
        }
        ...
        if($configHandler->insertConfig($config))
        {
            $log->addReport(
                XCube_Utils::formatString(
                    _MI_BIZFORUM_INSTALL_MSG_CONFIG_UPDATED,
                    $config->get('conf_name')
                )
            );
            return true;
        ...
        $log->addError(
            XCube_Utils::formatString(
                _MI_BIZFORUM_INSTALL_ERROR_CONFIG_UPDATED,
                $config->get('conf_name')
            )
        );
        return false;
    ...
    **/
    public static function updateConfigOrderByInfo(/*** Legacy_PreferenceInformation ***/ &$info,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(!(count($configs) > 0 && is_object($configs[0])))
        {
            $log->addError(_MI_BIZFORUM_INSTALL_ERROR_CONFIG_NOT_FOUND);
            return false;
        }
