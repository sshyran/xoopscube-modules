<?php
/**
 * @version $Id: onuninstall.php 105 2006-12-04 04:26:38Z hodaka $
 */

eval( ' function xoops_module_uninstall_'.$mydirname.'( $module ) { return treemenu_onuninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'treemenu_onuninstall_base' ) ) {

function treemenu_onuninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleUninstall.' . ucfirst($mydirname) . '.Success' , 'treemenu_message_append_onuninstall' ) ;
		$ret = array() ;
	} else {
	...
	$sql_file_path = dirname(dirname(__FILE__)).'/sql/mysql.sql' ;
...
function treemenu_message_append_onuninstall( &$module_obj , &$log )
{
