...
function Dbkmarken_GetTagCloud($where = null, $min = 80, $max = 200, $dirname)
{
	$wh = "";
	$ret = "";
	$tags = array();
	...
	$sql = "SELECT tag_name AS tag, COUNT(tag_id) AS quantity
	  FROM ". $db->prefix($dirname ."_tag") .$wh. "
	  GROUP BY tag_name
	...
	foreach ($tags as $key => $value) {
	    ...
	    $ret = $ret .'<a href="'. XOOPS_URL .'/modules/'. $dirname .'/index.php?action=TagList&tag_name='. urlencode($key) .'" style="font-size: '.$size.'%">'.$key.'</a> ';
