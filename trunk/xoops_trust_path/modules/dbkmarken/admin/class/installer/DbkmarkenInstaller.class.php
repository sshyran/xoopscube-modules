...
class Dbkmarken_Installer
{
    ...
    **/
    ...
    {
        return Dbkmarken_InstallUtils::installSQLAutomatically(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    private function _installModule()
    {
        ...
        {
            $this->mLog->addError(_MI_DBKMARKEN_INSTALL_ERROR_MODULE_INSTALLED);
            return false;
        }
        ...
        if($this->_mXoopsModule->getInfo('hasAdmin'))
        {
            ...
            {
                $this->mLog->addError(_MI_DBKMARKEN_INSTALL_ERROR_PERM_ADMIN_SET);
            }
        }
        ...
        if($this->_mXoopsModule->getInfo('hasMain'))
        {
            if($this->_mXoopsModule->getInfo('read_any'))
            {
                    ...
                    foreach($groupObjects as $group)
                    {
                        ...
                        {
                            $this->mLog->addError(_MI_DBKMARKEN_INSTALL_ERROR_PERM_READ_SET);
                        }
                    }
            }
            else
    ...
    **/
    ...
    {
        Dbkmarken_InstallUtils::installAllOfModuleTemplates(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    ...
    {
        Dbkmarken_InstallUtils::installAllOfBlocks(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    ...
    {
        Dbkmarken_InstallUtils::installAllOfConfigs(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    private function _processReport()
    {
        if(!$this->mLog->hasError())
        {
            $this->mLog->add(
                XCube_Utils::formatString(
                    _MI_DBKMARKEN_INSTALL_MSG_MODULE_INSTALLED,
        ...
        else if(is_object($this->_mXoopsModule))
        {
            $this->mLog->addError(
                XCube_Utils::formatString(
                    _MI_DBKMARKEN_INSTALL_ERROR_MODULE_INSTALLED,
                    $this->_mXoopsModule->getInfo('name')
                )
            );
        }
