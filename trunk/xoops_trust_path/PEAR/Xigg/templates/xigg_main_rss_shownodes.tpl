...
<rdf:RDF
 ...
 <channel rdf:about="<?php echo $this->Request->createUri(array('base' => '/rss'));?>">
  <title><?php _h($sitename);?><?php if(isset($requested_category)):?> (<?php _h($requested_category->getLabel());?>)<?php endif;?></title>
  <link><?php if(!isset($requested_category)):?><?php echo $this->Request->createUri(array('params' => array('keyword' => $requested_keyword)));?><?php else:?><?php echo $this->Request->createUri(array('params' => array('category_id' => $requested_category->getId(), 'keyword' => $requested_keyword)));?><?php endif;?></link>
...
<?php if (isset($nodes)):?>
<?php   $nodes =& $nodes->with('Tags'); $nodes =& $nodes->with('User'); $nodes->rewind();?>
<?php   while ($node =& $nodes->getNext()):?>
