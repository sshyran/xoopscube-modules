...
function setup_package($dirname) {
    ...
    echo "<div class='adminnavi'>".htmlspecialchars($title)."</div>\n";
...
function list_packages() {
    echo "<div class='adminnavi'>"._AM_REG_PACKAGES."</div>\n";
    echo "<div class='tips'>"._AM_REG_DESCRIPTION."</div>\n";
	...
	foreach ($pkgs as $dirname => $pkg) {
		...
		$check."</td><td>$qname</td><td align='center'>".
		htmlspecialchars($pkg['pversion'])."</td><td>".
		htmlspecialchars($pkg['name'])."</td><td>$dirname</td><td align='center'>".
		$date."</td><td align='center'><a href='?op=detail&dir=$dirname'><img src='".XOOPS_URL."/images/icons/information.png' alt='"._AM_DETAIL."' title='"._AM_DETAIL."' /></a></td></tr>\n";
	}
	echo "<tr><td colspan='7' class='foot'><input name='regpkg' type='submit' value='"._AM_REG_SUBMIT."'/></td></tr></table>\n";
	echo "</form>\n";
    } else {
