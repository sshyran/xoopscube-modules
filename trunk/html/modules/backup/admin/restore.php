...
//	Display Header
xoops_cp_header();
print '<div class="adminnavi"><a href="./index.php" title="'._AM_MYXBACKUP_GOBACK_TO_MENU.'">BackUp</a> &raquo;&raquo; Restore</div>';
if (MYXBU_DEBUG) { print '<div class="confirmMsg">DEBUG MODE</div>'; }
...
if (isset($_POST['op']) && ($_POST['op'] == 'restore')) {
	...
	if ($jobstat) {
		...
		print '<div class="confirmMsg">';
	}
	print _AM_MYXBACKUP_RESTORE_DONE.'</div>';
} else {
	//	Confirmation
	print '<table class="outer" width="100%">';
	...
	print '<table class="outer" width="100%"><tr><th colspan="2">'._AM_MYXBACKUP_GO_RESTORE_NOTE.'</th></tr>';
	print '<tr><td class="head">'._AM_MYXBACKUP_MYPREFIX.'</td>';
	print '<td class="odd">'.XOOPS_DB_PREFIX.'</td></tr>';
	print '<tr><td class="head">'._AM_MYXBACKUP_SQL_PREFIX.'</td>';
	print '<td class="even"><input type="text" name="prefix" size="24" value="'.XOOPS_DB_PREFIX.'" /><br />'._AM_MYXBACKUP_SQLPREFIX.'</td></tr>';
	print '<tr><td class="head">'._AM_MYXBACKUP_GO_RESTORE.'</td>';
	print '<td class="foot"><input type="hidden" name="op" value="restore" />';
