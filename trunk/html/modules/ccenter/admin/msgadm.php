...
LEFT JOIN $users u ON touid=u.uid LEFT JOIN $users f ON m.uid=f.uid";
    ...
    echo "<div class='resultMsg'>"._AM_MSG_COUNT." $total</div>\n";
    echo "<p align='center'>".$nav->renderNav()."</p>\n";
  ...
  <form method='get'>"._SEARCH."
	...
	foreach ($listctrl->getLabels($labels) as $lab) {
		...
		$anc = " <a href='?$args' title='"._CC_SORT_ORDER."'$extra><img src='../images/".$lab['value'].".png'></a>";
	    } else $anc = '';
